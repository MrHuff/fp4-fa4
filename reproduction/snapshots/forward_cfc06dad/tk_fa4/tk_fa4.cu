...
std::vector<at::Tensor> b300_mha_bwd_hot_cute16_candidate_s65536h64_loader_owned_dk_q_internal(
    ...
    TORCH_CHECK(
        causal,
        "exact S65536 H64 loader-owned dK-Q route supports causal=True only"
    );
    TORCH_CHECK(
        !deterministic,
        "exact S65536 H64 loader-owned dK-Q route supports deterministic=False only"
    );
    TORCH_CHECK(
        actual_seq_len == q.size(1),
        "exact S65536 H64 loader-owned dK-Q route does not support sequence padding"
    );
    TORCH_CHECK(
        q.size(0) == 1 && actual_seq_len == 65536 && q.size(2) == 64,
        "exact S65536 H64 loader-owned dK-Q route supports B1 S65536 H64 only"
    );
