...
def prepare_nvfp4_tk_inputs(
    ...
    dim = packed_dim * 2
    if dim not in (64, 128) or tuple(k_fp4.shape) != tuple(q_fp4.shape):
        raise ValueError("NVFP4 direct QK requires matching D64 or D128 Q and K")
    expected_scale = (
        32,
        4,
        seqlen // 128,
...
def main() -> None:
    ...
    if args.dim == 64 and args.qk_format != "nvfp4":
        raise ValueError("D64 is supported only by the NVFP4 QK atom")
    (
        q_fp4,
        k_fp4,
        v_fp8,
    ...
    tk_output = torch.empty(
        (args.batch, args.seqlen, args.heads, args.dim),
        device="cuda",
