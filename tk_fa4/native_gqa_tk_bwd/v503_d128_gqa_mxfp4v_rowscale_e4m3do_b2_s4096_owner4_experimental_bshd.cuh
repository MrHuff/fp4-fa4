...
// Experimental B2/S4096-only row-normalized MXFP4-V x E4M3-dO dP fork.
// Preserve v490's owner-4/direct-dK-dV topology and gradient publication.
// Only dP changes: each row's four block-32 scales are collapsed to their
// maximum once during the persistent V restage, with an exact power-of-two
// E2M1 RNE requantization.  The resulting scale-free E2M1 tile uses the
// unscaled mixed f8f6f4 instruction; a per-row FP32 factor restores v490's
// x16 dP domain in the existing compact dS epilogue.  This file is
// deliberately fail-closed to B2/S4096.
namespace tkfa4::native_gqa_tk_bwd::v503_d128_gqa_mxfp4v_rowscale_e4m3do_b2_s4096_owner4_experimental_bshd {
...
// f8f6f4 consumes aligned narrow operands: each eight bytes of packed E2M1
...
struct alignas(16) row_scale_state {
    float dp_factor[kKeyTile];
    uint8_t common_e8m0[kKeyTile];
};
static_assert(sizeof(row_scale_state) == 640);
...
__device__ __forceinline__ uint32_t make_ds_compact_word(
    ...
    // The unscaled MMA accumulates normalized E2M1(V) * E4M3(x4 dO).
    // `dp_factor=(2/3)*2^(common_code-127)` restores v490's x16 domain;
    // dstat is already x16 and must remain outside that multiplication.
    first.x = fmaf(first.x, dp_factor, statistic_first.x) * probability.x;
    first.y = fmaf(first.y, dp_factor, statistic_first.y) * probability.y;
    second.x =
        fmaf(second.x, dp_factor, statistic_second.x) * probability.z;
    second.y =
        fmaf(second.y, dp_factor, statistic_second.y) * probability.w;
    first.x *= beta;
    first.y *= beta;
    second.x *= beta;
    second.y *= beta;
...
__device__ __forceinline__ void consume_ds_half_compact(
    ...
    int column_half,
    int input_stage,
    const row_scale_state &v_row_scale,
    float beta
) {
    ...
    const int lane_row = laneid() & 15;
    const float dp_factor =
        v_row_scale.dp_factor[output_subtile * 16 + lane_row];
    const int lane_column_base = 32 * (laneid() >> 4);
    const int query_column_base =
        column_half * kColumnHalf + lane_column_base;
    const uint32_t shared_address = static_cast<uint32_t>(
...
// Map one signed E2M1 nibble to RNE(E2M1 * 2^-exponent_delta).
// Magnitude codes are {0,.5,1,1.5,2,3,4,6}; ties select the even code.
// A delta >= 5 maps every finite E2M1 value to canonical positive zero.
constexpr uint64_t kE2m1RneShift0 = 0xfedcba9076543210ull;
constexpr uint64_t kE2m1RneShift1 = 0xdcbaa90054322100ull;
constexpr uint64_t kE2m1RneShift2 = 0xbaa9900032211000ull;
constexpr uint64_t kE2m1RneShift3 = 0xa990000021100000ull;
constexpr uint64_t kE2m1RneShift4 = 0x9000000010000000ull;
constexpr uint8_t e2m1_lut_nibble(uint64_t lookup, int nibble) {
    return static_cast<uint8_t>((lookup >> (4 * nibble)) & 0xfu);
}
static_assert(e2m1_lut_nibble(kE2m1RneShift0, 7) == 7);
static_assert(e2m1_lut_nibble(kE2m1RneShift0, 8) == 0);
static_assert(e2m1_lut_nibble(kE2m1RneShift1, 1) == 0);
static_assert(e2m1_lut_nibble(kE2m1RneShift1, 3) == 2);
static_assert(e2m1_lut_nibble(kE2m1RneShift4, 7) == 1);
static_assert(e2m1_lut_nibble(kE2m1RneShift4, 15) == 9);

__device__ __forceinline__ uint64_t select_e2m1_rne_lookup(
    int exponent_delta
) {
    switch (exponent_delta) {
        case 0:
            return kE2m1RneShift0;
        case 1:
            return kE2m1RneShift1;
        case 2:
            return kE2m1RneShift2;
        case 3:
            return kE2m1RneShift3;
        case 4:
            return kE2m1RneShift4;
        default:
            return 0;
    }
}

__device__ __forceinline__ uint64_t requantize_e2m1_u64_rne(
    uint64_t payload,
    int exponent_delta
) {
    const uint64_t lookup = select_e2m1_rne_lookup(exponent_delta);
    if (lookup == 0) {
        return 0;
    }
    uint64_t result = 0;
#pragma unroll
    for (int byte_index = 0; byte_index < 8; ++byte_index) {
        const uint8_t packed = static_cast<uint8_t>(
            payload >> (8 * byte_index)
        );
        const uint8_t low = static_cast<uint8_t>(
            (lookup >> (4 * (packed & 0xfu))) & 0xfu
        );
        const uint8_t high = static_cast<uint8_t>(
            (lookup >> (4 * (packed >> 4))) & 0xfu
        );
        result |= static_cast<uint64_t>(low | (high << 4)) <<
            (8 * byte_index);
    }
    return result;
}
...
// required by unscaled f8f6f4.  Before expansion, each row's four block-32
// scales are collapsed to the maximum.  Every original nibble is then RNE
// requantized by the exact power-of-two ratio to that common scale.  This is a
// one-time persistent CTA restage; no scale TMEM is needed in the iteration
// loop.  The physical scale-page mapping is the projection producer's exact
// [row_in_32, tile_in_block, k_block] mapping.
__device__ __forceinline__ void stage_persistent_rowscale_mxfp4_v(
    const globals &g,
    shared_storage &storage,
    row_scale_state &row_scale,
    int batch,
    int key_tile,
    int kv_head
) {
    ...
    const size_t scale_page_index =
        (static_cast<size_t>(batch) * (g.sequence / kKeyTile) + key_tile) *
            kKvHeads +
        kv_head;
    const auto *scale_page = reinterpret_cast<const uint8_t *>(
        g.v_backward_mxfp4_scales.raw_ptr + scale_page_index * 512
    );

    if (threadIdx.x < kKeyTile) {
        const int row = static_cast<int>(threadIdx.x);
        const int row_in_32 = row & 31;
        const int tile_in_block = row >> 5;
        const int scale_base = row_in_32 * 16 + tile_in_block * 4;
        uint8_t common_code = scale_page[scale_base];
#pragma unroll
        for (int k_block = 1; k_block < 4; ++k_block) {
            common_code = max(common_code, scale_page[scale_base + k_block]);
        }
        row_scale.common_e8m0[row] = common_code;
        // E8M0 code zero is the producer's all-zero sentinel.  For codes
        // 1..254, the IEEE exponent field is exactly 2^(code-127).  Because
        // common_code is a max, common_code==0 proves all four blocks are
        // zero and factor zero is exact.  If only one source block has code
        // zero while another sets a nonzero common code, its producer payload
        // is already zero; the large delta also canonicalizes it to zero.
        const float power_of_two = common_code == 0
            ? 0.0f
            : __uint_as_float(static_cast<uint32_t>(common_code) << 23);
        row_scale.dp_factor[row] = (2.0f / 3.0f) * power_of_two;
    }
    __syncthreads();

    const auto *v_bytes = reinterpret_cast<const uint8_t *>(
        g.v_backward_mxfp4.raw_ptr
    );
    ...
    for (int segment = static_cast<int>(threadIdx.x);
        ...
        const int k_block = row_segment >> 1;
        const int row_in_32 = row & 31;
        const int tile_in_block = row >> 5;
        const uint8_t source_code = scale_page[
            row_in_32 * 16 + tile_in_block * 4 + k_block
        ];
        const int exponent_delta =
            static_cast<int>(row_scale.common_e8m0[row]) -
        ...
        const uint64_t normalized = requantize_e2m1_u64_rne(
            payload,
            exponent_delta
        );
        ...
        *reinterpret_cast<uint64_t *>(destination) = normalized;
    }
}

constexpr uint32_t make_unscaled_mixed_dp_instruction_descriptor() {
    return
        (1u << 4) |       // FP32 accumulator
        (5u << 7) |       // A format: E2M1
        (0u << 10) |      // B format: E4M3
        (0u << 15) |      // A K-major / not transposed
        (0u << 16) |      // B K-major / production mm_ABt descriptor mode
        (16u << 17) |     // N / 8 = 16
        (8u << 24);       // M / 16 = 8
}

// Independently spell every PTX instruction-descriptor field rather than
// relying on TK's same-type helper, which cannot express E2M1 x E4M3.
constexpr uint32_t kUnscaledMixedDpInstruction =
    make_unscaled_mixed_dp_instruction_descriptor();
static_assert(kUnscaledMixedDpInstruction == 0x08200290u);
static_assert(((kUnscaledMixedDpInstruction >> 7) & 7u) == 5u);
static_assert(((kUnscaledMixedDpInstruction >> 10) & 7u) == 0u);
static_assert(((kUnscaledMixedDpInstruction >> 17) & 0x3fu) == 16u);
static_assert(((kUnscaledMixedDpInstruction >> 24) & 0x1fu) == 8u);

// Native SM100 unscaled mixed dP: row-normalized E2M1 V times resident E4M3
// dO.  Both descriptors are K-major, matching production warpgroup::mm_ABt.
// f8f6f4 reduces K32, so both A and B advance exactly one descriptor chunk.
__device__ __forceinline__ void issue_rowscale_mxfp4v_e4m3do_dp(
...
void owner4_kernel(const __grid_constant__ globals g) {
    ...
    __shared__ alignas(16) row_scale_state v_row_scale_shared;
    ...
    stage_persistent_rowscale_mxfp4_v(
        g,
        storage,
        v_row_scale_shared,
        batch,
...
#pragma unroll 1
        for (int local_head = 0; local_head < kHeadsPerOwner; ++local_head) {
            ...
            issue_rowscale_mxfp4v_e4m3do_dp(
                dp_tmem,
                storage.v,
...
inline void launch(
    ...
    TORCH_CHECK(
        ...
        "v503 experimental MXFP4-V backward is fail-closed to B2/S4096"
    );
