...
// dQ/dK waits, wait for it, issue x16 chunk 1, compute/publish chunk 0 while
// chunk 1 is in flight, then wait for and publish chunk 1.  B1 falls back to
// v436; other B2 sequence lengths fall back to v437.
namespace tkfa4::native_gqa_tk_bwd::v459_d128_gqa_e4m3_b2_s4096_owner4_quarter_dp_split_dv_compact_p_reuse_production_bshd {
...
// Keep the compiler from sinking an asynchronous quarter load into the
// following quarter's arithmetic.  Naming every output preserves the complete
// x16 result across this compiler-only memory barrier; the generated program
// receives no extra numerical operation.
__device__ __forceinline__ void anchor_dp_quarter_issue(
    const owner_aligned_fp32_quarter &source
) {
    asm volatile(
        ""
        :
        : "f"(source.pairs[0].x), "f"(source.pairs[0].y),
          "f"(source.pairs[1].x), "f"(source.pairs[1].y),
          "f"(source.pairs[2].x), "f"(source.pairs[2].y),
          "f"(source.pairs[3].x), "f"(source.pairs[3].y),
          "f"(source.pairs[4].x), "f"(source.pairs[4].y),
          "f"(source.pairs[5].x), "f"(source.pairs[5].y),
          "f"(source.pairs[6].x), "f"(source.pairs[6].y),
          "f"(source.pairs[7].x), "f"(source.pairs[7].y)
        : "memory"
    );
}
...
#pragma unroll 1
        for (int local_head = 0; local_head < kHeadsPerOwner; ++local_head) {
            for (
                ...
                anchor_dp_quarter_issue(dp_second_quarter);
