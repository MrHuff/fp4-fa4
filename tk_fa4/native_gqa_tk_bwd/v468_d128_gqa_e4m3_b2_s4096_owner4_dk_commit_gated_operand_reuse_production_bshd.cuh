...
// B2/S4096 synchronization-elision experiment derived only from frozen v466.
...
// and all numerical operations.  PTX defines tcgen05.commit as tracking all
// prior same-thread asynchronous TCGEN operations.  The later dK commit is
// issued by the same tensor lane after both dV halves, so dK completion also
// proves dV completion for operand-stage reuse.  Remove only the issuer's
// redundant second wait; retain dv_ready for shared-P reuse and final drains.
// B1 falls back to v436; other B2 sequence lengths fall back to v437.
namespace tkfa4::native_gqa_tk_bwd::v468_d128_gqa_e4m3_b2_s4096_owner4_dk_commit_gated_operand_reuse_production_bshd {
