#include "v508_d128_gqa_nvfp4_score_e4m3_gradient_b1_exact_s4096_experimental_bshd.cuh"
...
namespace candidate =
    tkfa4::native_gqa_tk_bwd::v508_d128_gqa_nvfp4_score_e4m3_gradient_b1_exact_s4096_experimental_bshd;
...
void check_native_payload(
    ...
    TORCH_CHECK(
        ...
        name,
        " must be contiguous CUDA float4_e2m1fn_x2 [1,",
        heads,
...
void check_native_scale(
    ...
    TORCH_CHECK(
        tensor.scalar_type() == at::ScalarType::Float8_e4m3fn &&
            ...
            tensor.size(3) == 512,
        name,
        " must be contiguous CUDA float8_e4m3fn [1,",
        sequence_tiles,
        ",",
        heads * kDepthChunks,
        ",512]"
...
void check_global_scale(
    ...
    TORCH_CHECK(
        ...
        name,
        " must be contiguous CUDA FP32 [1,",
        heads,
        "]"
    );
}
...
void check_arguments(
    ...
    check_bshd(dout, "dout_e4m3", kE4m3, candidate::kQueryHeads);
...
void launch_checked(
    ...
    TORCH_CHECK(
        ...
        "v508 experimental native-score hybrid requires SM100"
    );
...
void main_nvfp4_score_e4m3_gradient_bshd_precomputed(
    at::Tensor q,
    at::Tensor k,
    at::Tensor v,
    at::Tensor dout,
...
void backward_nvfp4_score_e4m3_gradient_bshd_precomputed_out(
    at::Tensor q,
    at::Tensor k,
    at::Tensor v,
    at::Tensor dout,
...
pybind11::dict metadata() {
    ...
    result["gradient_qk_dtype"] = "float8_e4m3fn_represented_x4";
    result["v_dout_dtype"] = "float8_e4m3fn_represented_x4";
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
    module.def(
        "main_nvfp4_score_e4m3_gradient_bshd_precomputed",
        &main_nvfp4_score_e4m3_gradient_bshd_precomputed
    );
    module.def(
