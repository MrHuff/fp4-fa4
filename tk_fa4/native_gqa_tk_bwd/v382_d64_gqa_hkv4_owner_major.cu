#include "v382_d64_gqa_hkv4_owner_major.cuh"
...
namespace owner =
    tkfa4::native_gqa_tk_bwd::v382_d64_hkv4_owner_major;
...
void check_common(
    ...
    TORCH_CHECK(
        ...
        "V382 D64 GQA owner-major four-head backward requires SM100"
    );
    TORCH_CHECK(
...
std::vector<at::Tensor> backward_bf16_precomputed(
    ...
    const std::vector<int64_t> accum_sizes{
        q.size(0),
        owner::kSequence,
        owner::kPartialHeads,
        owner::kDepth,
    };
    at::Tensor dk_accum = at::empty(
        accum_sizes,
        q.options().dtype(at::kFloat)
    );
    at::Tensor dv_accum = at::empty(
        accum_sizes,
        q.options().dtype(at::kFloat)
    );
    ...
    const owner::partial_globals globals{
    ...
    owner::launch_owner_major_bf16(globals, stream);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
    module.def(
        "backward_bf16",
        &backward_bf16,
        "V382 D64 GQA BF16 backward, owner-major four-head BSHD"
    );
    module.def(
        "backward_bf16_precomputed",
        &backward_bf16_precomputed,
        "V382 D64 GQA owner-major four-head BSHD backward with delta"
    );
}
