...
// B2/S4096 head-boundary overlap experiment derived only from frozen v469.
// Preserve its owner-4/direct-dK-dV topology, compact exact-rounded E4M3 P
// reuse, first-half quarter-dP pipeline, split-dV publication, completion
// elision, and all numerical operations.  At the three local-head boundaries,
// score(next head) uses a disjoint TMEM page and may be issued once next Q is
// ready while reducers still drain old dQ.  Retain dq_drained as the acquire
// before dP(next head), because dP aliases the old dQ TMEM page.  B1 falls back
// to v436; other B2 sequence lengths fall back to v437.
namespace tkfa4::native_gqa_tk_bwd::v472_d128_gqa_e4m3_b2_s4096_owner4_head_score_before_dq_drain_production_bshd {
