...
// B2/S4096 probability-conversion cadence experiment derived byte-for-source
// from frozen v447.  Preserve v447's owner-4/direct-dK-dV schedule, two-half
// order, exact rounded E4M3 P reuse, half-0 dP preissue, and v2 shared-memory
// publication.  The only device-path change is to clamp/EX2 four FP32 pairs,
// immediately pack the resulting two exact E4M3 words, retain them for dS,
// and publish that word pair before advancing.  B1 falls back to v436; other
// B2 sequence lengths fall back to v437.
namespace tkfa4::native_gqa_tk_bwd::v456_d128_gqa_e4m3_b2_s4096_owner4_interleaved_exp2_pack_compact_p_reuse_production_bshd {
...
__device__ __forceinline__ void make_probability_half_compact(
    ...
    using probability_half_tile = decltype(destination);
    static_assert(probability_half_tile::rows == 16);
    static_assert(probability_half_tile::cols == kColumnHalf);
    static_assert(sizeof(typename probability_half_tile::dtype) == 1);
    static_assert(probability_half_tile::swizzle);
    static_assert(probability_half_tile::swizzle_bytes == 128);
    const uint32_t shared_address = static_cast<uint32_t>(
        __cvta_generic_to_shared(&destination.data[0])
    );

    // Preserve v447's element order and exact numerical operations, but
    // shorten the live range of each four-pair EX2 result group by converting
    // and publishing its two E4M3 words before computing the next group.
#pragma unroll
    for (int word_pair = 0; word_pair < kCompactProbabilityWords / 2;
         ++word_pair) {
        const int first_word = 2 * word_pair;
        const int first_pair = 2 * first_word;
#pragma unroll
        for (int pair_offset = 0; pair_offset < 4; ++pair_offset) {
            const int pair = first_pair + pair_offset;
        ...
        const uint32_t packed_first = d64::pack_owner_e4m3_word(
            probability.pairs[first_pair],
            probability.pairs[first_pair + 1]
        );
        const uint32_t packed_second = d64::pack_owner_e4m3_word(
            probability.pairs[first_pair + 2],
            probability.pairs[first_pair + 3]
        );
...
#pragma unroll 1
        for (int local_head = 0; local_head < kHeadsPerOwner; ++local_head) {
            for (
                ...
                d64::owner_aligned_fp32_half dp_first_half;
                wait(dp_ready, phase);
