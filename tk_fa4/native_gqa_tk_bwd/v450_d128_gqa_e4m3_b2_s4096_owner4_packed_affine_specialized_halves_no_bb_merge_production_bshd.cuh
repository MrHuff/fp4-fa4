...
// Isolated B2/S4096 compiler-policy experiment derived from frozen v448.  It
// keeps v448's compile-time half specialization, packed FP32x2 score/lstat
// affine, owner-4/direct-dK-dV, and rounded-E4M3 compact-P schedule.  Its
// Makefile alone disables ptxas basic-block merging to test whether both
// probability halves remain straight-line.  B1 falls back to v436; other B2
// sequence lengths fall back to v437.
namespace tkfa4::native_gqa_tk_bwd::v450_d128_gqa_e4m3_b2_s4096_owner4_packed_affine_specialized_halves_no_bb_merge_production_bshd {
...
__device__ __forceinline__ void make_probability_half_compact(
    ...
    const float2 affine_scale{beta_log2e, beta_log2e};
...
#pragma unroll
    for (int pair = 0; pair < kColumnHalf / 4; ++pair) {
        ...
        value = base_ops::mul::op<float2>(value, affine_scale);
        value = base_ops::sum::op<float2>(value, statistic);
        if (diagonal) {
...
#pragma unroll 1
        for (int local_head = 0; local_head < kHeadsPerOwner; ++local_head) {
            for (
                ...
                make_probability_half_compact<0>(
                ...
                make_probability_half_compact<1>(
