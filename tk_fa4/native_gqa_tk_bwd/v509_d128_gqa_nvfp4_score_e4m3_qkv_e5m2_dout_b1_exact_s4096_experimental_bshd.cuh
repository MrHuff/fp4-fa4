...
// Fail-closed B1/S4096 correctness experiment reusing v508's native-NVFP4
// score schedule, E4M3 Q/K/V gradient operands, and E5M2 dO. Score
// recomputation alone
...
namespace tkfa4::native_gqa_tk_bwd::v509_d128_gqa_nvfp4_score_e4m3_qkv_e5m2_dout_b1_exact_s4096_experimental_bshd {
...
using dout_tile = st_fp8e5m2<kKeyTile, core::kDepth>;
static_assert(sizeof(native_qk_tile) == 8 * 1024);
static_assert(sizeof(native_qk_scale_tile) == 1024);
static_assert(sizeof(dout_tile) == sizeof(operand_tile));
...
struct globals {
    ...
    using dout_gl = gl<
        ...
        -1,
        -1,
        tma::descriptor<dout_tile, dim::DEPTH>
    >;
    ...
    // Q/K/V remain represented E4M3. dO alone uses wider-range E5M2 while
    // retaining the same x4 encode / x0.25 decode convention.
    operand_gl q;
    operand_gl k;
    operand_gl v;
    dout_gl dout;
...
// This is byte-for-byte the v508 shared layout except for dO's element type.
// E4M3 and E5M2 are both one byte, so the schedule, TMA footprint, semaphore
// offsets, and occupancy contract remain unchanged.
struct shared_storage {
    operand_tile k;
    operand_tile v;
    operand_tile q[kInputStages];
    dout_tile dout[kInputStages];
...
static_assert(sizeof(shared_storage) == sizeof(prior::shared_storage));
...
// v509 changes only the dO-dependent dense tensor-core products.  The
// authenticated microgate proves the mixed E4M3-A x E5M2-B descriptor as the
// E4M3 descriptor with exactly the B-format bit (0x400) added.  Keep the
// semaphore commit local to this adapter because issue_mixed_product() emits
// only the four K32 MMA commands.
__device__ __forceinline__ void issue_dp_e4m3_e5m2(
    attention_tmem_tile &destination,
    const operand_tile &v,
    const dout_tile &dout,
    semaphore &completion
) {
    mixed::issue_mixed_product<mixed::operation::dp_abt, 0>(
        destination,
        v,
        dout
    );
...
__device__ __forceinline__ void issue_dv_e4m3_e5m2_runtime_accumulate(
    gradient_tmem_tile &destination,
    const attention_tile &probability,
    const dout_tile &dout,
    semaphore &completion,
    bool accumulate
) {
    if (accumulate) {
        mixed::issue_mixed_product<mixed::operation::dv_ab, 1>(
            destination,
            probability,
            dout
        );
    } else {
        mixed::issue_mixed_product<mixed::operation::dv_ab, 0>(
            destination,
            probability,
            dout
        );
...
void b1_native_nvfp4_score_e4m3_qkv_e5m2_dout_exact_s4096_kernel(
...
#pragma unroll 1
        for (int local_head = 0; local_head < kHeadsPerOwner; ++local_head) {
            ...
            for (
                ...
                tensor_after_thread_sync();
                issue_dv_e4m3_e5m2_runtime_accumulate(
                    dv_tmem,
                    storage.probability,
                    storage.dout[stage],
                    dv_ready,
...
inline globals make_globals(
    ...
    return globals{
        ...
        kittens::py::tensor_to_gl<globals::dout_gl>(dout),
        ...
        kittens::py::tensor_to_gl<globals::global_scale_gl>(
            q_global_scale, q.size(0), kQueryHeads, 1, 1
        ),
        kittens::py::tensor_to_gl<globals::global_scale_gl>(
            k_global_scale, q.size(0), kKvHeads, 1, 1
        ),
        beta,
...
inline void launch(
    ...
    TORCH_CHECK(
        q.size(0) == TKFA4_V509_EXACT_BATCH &&
            q.size(1) == kExactSequence,
        "v509 core batch differs from its separately compiled exact batch"
    );
    ...
    b1_native_nvfp4_score_e4m3_qkv_e5m2_dout_exact_s4096_kernel<<<
