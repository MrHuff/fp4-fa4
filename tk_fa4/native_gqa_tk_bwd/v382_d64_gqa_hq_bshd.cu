#include "v382_d64_gqa_hq_bshd.cuh"
...
namespace owner = tkfa4::native_gqa_tk_bwd::v382_d64_hq_bshd;
...
void check_common(
    ...
    TORCH_CHECK(
        ...
        "V382 D64 GQA Hq-parallel BSHD backward requires SM100"
    );
    TORCH_CHECK(
...
std::vector<at::Tensor> backward_bf16_precomputed(
    ...
    owner::launch_hq_owner_bf16(globals, stream);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
    module.def(
        "backward_bf16",
        &backward_bf16,
        "V382 D64 GQA BF16 backward, Hq-parallel native BSHD"
    );
    module.def(
        "backward_bf16_precomputed",
        &backward_bf16_precomputed,
        "V382 D64 GQA Hq-parallel BSHD backward with delta"
    );
}
