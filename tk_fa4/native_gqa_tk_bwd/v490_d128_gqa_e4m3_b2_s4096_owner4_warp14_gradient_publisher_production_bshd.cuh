...
// B2/S4096 dedicated gradient-publisher experiment derived only from v482.
...
// handoff.  Retain exact-packed BF16 D0 and D1 in thirty-two registers, then
// issue the paired D2/D3 x32 loads.  Reducers retain the frozen one-tile
// capture/store path, while previously idle physical warp 14 owns every dQ,
// dK, and dV TMA issue plus source-read retirement.  The publisher signals
// shared reuse only after read_wait<0>, allowing reducer work to overlap the
// store stream without placing TMA state in the capture path.  B1 falls back
// to v436; other B2 sequence lengths fall back to v437.
namespace tkfa4::native_gqa_tk_bwd::v490_d128_gqa_e4m3_b2_s4096_owner4_warp14_gradient_publisher_production_bshd {
