...
// D128 exact-native production backward derived mechanically from v424.  It
// preserves v424's schedule, synchronization, and probability numerics while
// using fixed K128/Q128 shared descriptors and issuing TCGEN directly
// from the already-selected physical warp 12, lane 0.  The fixed issuer helpers
// deliberately contain no nested lane election or generic MMA dispatch.
namespace tkfa4::native_gqa_tk_bwd::v427_d128_gqa_e4m3_fixed_issuer_async_production_bshd {
...
using operand_n_descriptor =
    kittens::st_descriptor<operand_tile, kittens::transpose::N>;
using operand_t_descriptor =
    kittens::st_descriptor<operand_tile, kittens::transpose::T>;
using attention_n_descriptor =
    kittens::st_descriptor<attention_tile, kittens::transpose::N>;
using attention_t_descriptor =
    kittens::st_descriptor<attention_tile, kittens::transpose::T>;
...
// These helpers are reachable only from main_kernel's physical-warp-12/lane-0
// arm.  TCGEN05 MMA is a single-thread issue operation: adding another generic
// warpgroup lane predicate here merely nests the already-proven issuer role.
// Keep the exact v424 command decomposition, accumulator bits, proxy fence, and
// one completion commit per logical matrix product.
__device__ __forceinline__ void issue_fixed_score_or_dp(
    ...
    constexpr uint32_t instruction =
        kittens::detail::tcgen05::instruction_descriptor<
            ...
            false
    >();
    static_assert(instruction == 0x08200010u);
    operand_n_descriptor lhs_descriptor(lhs);
    operand_n_descriptor rhs_descriptor(rhs);
    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
    kittens::detail::tcgen05::st_st<fp8e4m3, 0, 1>(
        destination.addr,
        lhs_descriptor.chunk_descriptor(0),
        rhs_descriptor.chunk_descriptor(0),
        instruction
    );
    kittens::detail::tcgen05::st_st<fp8e4m3, 1, 1>(
        destination.addr,
        lhs_descriptor.chunk_descriptor(1),
        rhs_descriptor.chunk_descriptor(1),
        instruction
    );
    kittens::detail::tcgen05::st_st<fp8e4m3, 1, 1>(
        destination.addr,
        lhs_descriptor.chunk_descriptor(2),
        rhs_descriptor.chunk_descriptor(2),
        instruction
    );
    kittens::detail::tcgen05::st_st<fp8e4m3, 1, 1>(
        destination.addr,
        lhs_descriptor.chunk_descriptor(3),
        rhs_descriptor.chunk_descriptor(3),
        instruction
    );
    kittens::detail::tcgen05::commit<1>(completion);
}

template <int Accumulate>
__device__ __forceinline__ void issue_fixed_gradient_ab(
    ...
    constexpr uint32_t instruction =
        kittens::detail::tcgen05::instruction_descriptor<
            ...
            false
    >();
    static_assert(instruction == 0x08210010u);
    attention_n_descriptor lhs_descriptor(lhs);
    operand_t_descriptor rhs_descriptor(rhs);
    kittens::detail::tcgen05::st_st<fp8e4m3, Accumulate, 1>(
        destination.addr,
        lhs_descriptor.chunk_descriptor(0),
        rhs_descriptor.chunk_descriptor(0),
        instruction
    ...
    kittens::detail::tcgen05::st_st<fp8e4m3, 1, 1>(
        destination.addr,
        lhs_descriptor.chunk_descriptor(2),
        rhs_descriptor.chunk_descriptor(4),
        instruction
    );
    kittens::detail::tcgen05::st_st<fp8e4m3, 1, 1>(
        destination.addr,
        lhs_descriptor.chunk_descriptor(3),
        rhs_descriptor.chunk_descriptor(6),
        instruction
    );
    kittens::detail::tcgen05::commit<1>(completion);
}

__device__ __forceinline__ void issue_fixed_gradient_atb(
    gradient_tmem_tile &destination,
    const attention_tile &lhs,
    const operand_tile &rhs,
    semaphore &completion
    ...
    constexpr uint32_t instruction =
        kittens::detail::tcgen05::instruction_descriptor<
            ...
            false
    >();
    static_assert(instruction == 0x08218010u);
    attention_t_descriptor lhs_descriptor(lhs);
    operand_t_descriptor rhs_descriptor(rhs);
    kittens::detail::tcgen05::st_st<fp8e4m3, 0, 1>(
        destination.addr,
        lhs_descriptor.chunk_descriptor(0),
    ...
    kittens::detail::tcgen05::st_st<fp8e4m3, 1, 1>(
        destination.addr,
        lhs_descriptor.chunk_descriptor(4),
        rhs_descriptor.chunk_descriptor(4),
        instruction
    );
    kittens::detail::tcgen05::st_st<fp8e4m3, 1, 1>(
        destination.addr,
        lhs_descriptor.chunk_descriptor(6),
        rhs_descriptor.chunk_descriptor(6),
        instruction
    );
    kittens::detail::tcgen05::commit<1>(completion);
...
inline void launch(
    ...
    v427_d128_gqa_e4m3_fixed_issuer_async_production_bshd::main_kernel<<<
        grid,
        kThreads,
        0,
        stream
