...
// D128 exact-native production backward with an owner-aligned dQ evacuation
// path.  Four reducer warps use one full-row tcgen05 x32 load apiece for each
// D32 slice and first move the complete FP32 D128 payload to shared memory.
// The collective dQ lifetime gate can therefore release TMEM before BF16
// conversion and additive-TMA publication.  Score release and all numerical
// behavior otherwise remain identical to v424.
namespace tkfa4::native_gqa_tk_bwd::v428_d128_gqa_e4m3_owner_dq_evacuation_production_bshd {
...
using dq_evacuation_tile = st_fl<kKeyTile, kDepth>;
...
struct shared_storage {
    ...
    dq_evacuation_tile dq_evacuation;
    stats_tile lstat[kInputStages];
    stats_tile dstat[kInputStages];
};

// v424's 146 KiB payload plus one 128x128 FP32 dQ evacuation tile (64 KiB).
// The complete kernel adds 1,312 bytes of semaphore state, for 216,352 bytes
// total: below SM100's 232,448-byte per-block opt-in limit with one CTA/SM.
static_assert(sizeof(shared_storage) == 210 * 1024);
...
// One reducer warp owns 32 physical rows.  tcgen05's 32x32b.x32 lane mapping
// gives each lane the complete D32 row selected by src_addr's row field.  This
// is the same mapping used by the authenticated CuTe boundary and by the
// project's fused dense dQ publisher; keeping the helper local avoids changing
// any shared TK primitive.
...
// Evacuate the complete 128x128 FP32 dQ page before releasing its TMEM
// lifetime.  Only one D32 row fragment is live at a time, avoiding the
// impossible 128-register-per-lane full-width register materialization.
__device__ __forceinline__ void evacuate_dq_to_shared(
    const gradient_tmem_tile &source,
    dq_evacuation_tile &destination,
...
#pragma unroll 1
    for (int depth_chunk = 0; depth_chunk < kDepthChunks; ++depth_chunk) {
        ...
        tensor_load_wait();
#pragma unroll
        for (int column = 0; column < kDepthChunk; column += 4) {
            move<float4>::sts(
                dq_evacuation_tile::idx(
                    destination_base,
                    {
                        physical_row,
    ...
    // Every lane has completed all four TMEM reads before its warp contributes
    // one arrival.  The issuer observes all four reducer-warp arrivals before
    // overwriting [0,128) with dP(next).
    tensor_before_thread_sync();
    __syncwarp();
...
// Convert one owner-aligned D32 slice from the evacuation tile into the same
// swizzled BF16 stage consumed by the v424 additive x4 TMA ABI.
__device__ __forceinline__ void stage_evacuated_dq_chunk(
    const dq_evacuation_tile &source,
    ...
    const int physical_row = logical_warp * 32 + lane;
    const uint32_t source_base = static_cast<uint32_t>(
        __cvta_generic_to_shared(source.data)
    );
    const uint32_t destination_base = static_cast<uint32_t>(
        __cvta_generic_to_shared(destination.data)
    );
    ...
    for (int column = 0; column < kDepthChunk; column += 8) {
        ...
        move<float4>::lds(
            ...
            dq_evacuation_tile::idx(
                ...
                {
                    physical_row,
                    depth_chunk * kDepthChunk + column + 4,
                }
            )
        );
        const bf16_2 packed0 = __floats2bfloat162_rn(
            low.x * kOutputScale,
            low.y * kOutputScale
        );
        const bf16_2 packed1 = __floats2bfloat162_rn(
            low.z * kOutputScale,
            low.w * kOutputScale
        );
        const bf16_2 packed2 = __floats2bfloat162_rn(
            high.x * kOutputScale,
            high.y * kOutputScale
        );
        const bf16_2 packed3 = __floats2bfloat162_rn(
            high.z * kOutputScale,
            high.w * kOutputScale
        );
        store_bf16_x8(
...
// Publication deliberately mirrors v421/v424.  The sole change is that the
// source has already left TMEM, so conversion and TMA issue are no longer on
// the dP(next) lifetime path.
...
inline void launch(
    ...
    v428_d128_gqa_e4m3_owner_dq_evacuation_production_bshd::main_kernel<<<
        grid,
        kThreads,
        0,
        stream
