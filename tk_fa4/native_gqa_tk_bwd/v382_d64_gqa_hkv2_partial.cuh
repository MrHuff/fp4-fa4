...
// Disjoint-partial epilogue for the two-Hq-head V382 owner topology.
// Compute ownership is identical to hkv2_owner, but each split writes a
// distinct FP32 [B,S,Hkv*2,D] tile.  A group-2 finalizer performs the only
// cross-split reduction, avoiding FP32 TMA atomic stores in the main kernel.

#include "v382_d64_gqa_hkv2_owner.cuh"

namespace tkfa4::native_gqa_tk_bwd::v382_d64_hkv2_partial {

namespace control = v382_d64_owner;
namespace hkv2 = v382_d64_hkv2_owner;
...
using hkv2::kHeadsPerSplit;
using hkv2::kSplitsPerKvHead;

constexpr int kPartialHeads = kKvHeads * kSplitsPerKvHead;

struct partial_globals {
    using q_gl = hkv2::split_globals::q_gl;
    using k_gl = hkv2::split_globals::k_gl;
    using v_gl = hkv2::split_globals::v_gl;
    using dout_gl = hkv2::split_globals::dout_gl;
    using dq_gl = hkv2::split_globals::dq_gl;
    using partial_gl = gl<
        float,
        -1,
        -1,
...
template <int HeadsPerOwner, bool OwnerMajor = false>
__global__ __launch_bounds__(kThreads, 1)
void owner_partial_bf16_kernel(
    const __grid_constant__ partial_globals globals
) {
    static_assert(
        HeadsPerOwner > 0 && kHeadRatio % HeadsPerOwner == 0
    );
    constexpr int kOwnersPerKvHead = kHeadRatio / HeadsPerOwner;
    ...
    constexpr int kOwnerHeads = kQueryHeads / HeadsPerOwner;
    const int cta_rank = static_cast<int>(blockIdx.x) & 1;
    const int cluster_linear = static_cast<int>(blockIdx.x) >> 1;
    const int owner_idx = OwnerMajor
        ? cluster_linear / kOwnerHeads
        : cluster_linear;
    const int partial_head = OwnerMajor
        ? cluster_linear % kOwnerHeads
        : static_cast<int>(blockIdx.y);
    const int kv_head = partial_head / kOwnersPerKvHead;
    const int ratio_base =
        (partial_head % kOwnersPerKvHead) * HeadsPerOwner;
...
#pragma unroll
    for (int local_head = 0; local_head < HeadsPerOwner; ++local_head) {
...
void reduce_pair_partials_kernel(
    ...
    for (
        ...
        const int split_head = kv_head * kSplitsPerKvHead;
        const int64_t first =
            ((batch * kSequence + sequence) * kPartialHeads + split_head) *
                kDepth +
            depth;
...
inline void launch_hkv2_partial_bf16(
    const partial_globals &globals,
    cudaStream_t stream
) {
    kittens::LaunchConfig<true, false> launch_config(
        dim3(kOwnerClusters * 2, kPartialHeads, globals.batch),
        dim3(kThreads, 1, 1),
        0,
        stream,
        dim3(2, 1, 1)
    ...
    CUDACHECK(cudaLaunchKernelEx(
        launch_config,
        owner_partial_bf16_kernel<kHeadsPerSplit>,
        globals
    ));
}
