...
// Register-resident score/probability/dP specialization of the validated
// D64 GQA Hkv-owner control.
//
...
namespace tkfa4::native_gqa_tk_bwd::v382_d64_hkv_register_pd {

namespace base = v382_d64_hkv_owner;
namespace control = v382_d64_owner;

using base::dkdv_bf16_stage_tile;
using base::hkv_globals;
...
using attention_bf16_register = rt_bf<16, 128>;
using attention_fp32_quarter_register = rt_fl<16, 32>;
using attention_bf16_quarter_register = rt_bf<16, 32>;
...
static_assert(sizeof(register_pd_storage) < 208 * 1024);
static_assert(
...
__device__ __forceinline__ void make_probability_quarter(
    attention_bf16_register &probability_ds,
    attention_fp32_register &score,
    const sv_fl<kQueryTile> &lse_log2,
    float scale_log2e
) {
    ...
    auto &probability_quarter = *reinterpret_cast<
        attention_bf16_quarter_register *
    >(&probability_ds.tiles[0][Quarter * 2]);
    attention_quarter_stats_register lse;
    warp::load(lse, lse_log2.template subvec<32>(Quarter));
    warp::mul(score_quarter, score_quarter, scale_log2e);
    warp::sub_col(score_quarter, score_quarter, lse);
    warp::exp2(score_quarter, score_quarter);
    warp::copy(probability_quarter, score_quarter);
}

// Center one dP quarter, multiply by the rounded probability retained in
// registers, fold in the softmax scale, and narrow in-place to the tensor-core
// dS operand.  Quartered statistics cut the peak register lifetime by 24
// registers per reducer thread relative to a full 128-column row vector.
template <int Quarter>
__device__ __forceinline__ void make_ds_quarter(
    attention_bf16_register &probability_ds,
    attention_fp32_register &dp,
    const sv_fl<kQueryTile> &delta,
    float scale
) {
    static_assert(Quarter >= 0 && Quarter < 4);
    ...
    auto &probability_ds_quarter = *reinterpret_cast<
        attention_bf16_quarter_register *
    >(&probability_ds.tiles[0][Quarter * 2]);
    attention_quarter_stats_register delta_quarter;
    warp::load(delta_quarter, delta.template subvec<32>(Quarter));
    warp::sub_col(dp_quarter, dp_quarter, delta_quarter);
    const float2 scale_pair{scale, scale};
#pragma unroll
    for (int tile_row = 0; tile_row < dp_quarter.height; ++tile_row) {
#pragma unroll
        for (
            int tile_column = 0;
            tile_column < dp_quarter.width;
            ++tile_column
        ) {
#pragma unroll
            for (
                int packed = 0;
                packed < dp_quarter.packed_per_tile;
                ++packed
            ) {
                const float2 p =
                    base_types::convertor<float2, bf16_2>::convert(
                        probability_ds_quarter
                            .tiles[tile_row][tile_column]
                            .data[packed]
                    );
                float2 value = base_ops::mul::op<float2>(
                    p,
                    dp_quarter.tiles[tile_row][tile_column].data[packed]
                );
                value = base_ops::mul::op<float2>(value, scale_pair);
                dp_quarter.tiles[tile_row][tile_column].data[packed] = value;
                probability_ds_quarter
                    .tiles[tile_row][tile_column]
                    .data[packed] =
...
void hkv_register_pd_bf16_kernel(
    const __grid_constant__ hkv_globals globals
) {
    __shared__ alignas(1024) register_pd_storage storage;
...
inline void launch_hkv_register_pd_bf16(
