...
namespace owner =
    tkfa4::native_gqa_tk_bwd::v382_d64_hkv2_owner;
...
void check_common(
    ...
    TORCH_CHECK(
        ...
        "V382 D64 GQA two-head owner backward requires SM100"
    );
    TORCH_CHECK(
...
std::vector<at::Tensor> backward_bf16_precomputed(
    ...
    at::Tensor dk_accum = at::zeros(
        k.sizes(),
        k.options().dtype(at::kFloat)
    );
    at::Tensor dv_accum = at::zeros(
        v.sizes(),
        v.options().dtype(at::kFloat)
    );
    at::Tensor dq = at::empty(q.sizes(), q.options());
    at::Tensor dk = at::empty(k.sizes(), k.options());
    at::Tensor dv = at::empty(v.sizes(), v.options());

    const owner::split_globals globals{
        kittens::py::tensor_to_gl<owner::split_globals::q_gl>(q),
        kittens::py::tensor_to_gl<owner::split_globals::k_gl>(k),
        kittens::py::tensor_to_gl<owner::split_globals::v_gl>(v),
        kittens::py::tensor_to_gl<owner::split_globals::dout_gl>(dout),
        kittens::py::tensor_to_gl<owner::split_globals::hkv_accum_gl>(
            dk_accum
        ),
        kittens::py::tensor_to_gl<owner::split_globals::hkv_accum_gl>(
            dv_accum
        ),
        kittens::py::tensor_to_gl<owner::split_globals::dq_gl>(dq_accum),
    ...
    owner::launch_hkv2_owner_bf16(globals, stream);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
    module.def(
        "backward_bf16",
        &backward_bf16,
        "V382 D64 GQA BF16 backward, two-head split native BSHD"
    );
    module.def(
        "backward_bf16_precomputed",
        &backward_bf16_precomputed,
        "V382 D64 GQA BF16 backward, two-head split BSHD with delta"
    );
}
