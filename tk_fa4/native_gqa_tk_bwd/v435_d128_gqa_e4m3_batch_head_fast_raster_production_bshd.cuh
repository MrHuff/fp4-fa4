...
// D128 batch-head-fast-raster experiment cloned from final v433.  It keeps
// v433's compute, native-EX2 numerics, score release, and early full-dQ
...
namespace tkfa4::native_gqa_tk_bwd::v435_d128_gqa_e4m3_batch_head_fast_raster_production_bshd {
...
void main_kernel(const __grid_constant__ globals g) {
    ...
    const int query_head = flat & 31;
    const int batch = flat >> 5;
...
inline void launch(
    ...
    const dim3 grid(
        static_cast<unsigned int>(q.size(0) * q.size(2)),
        static_cast<unsigned int>(q.size(1) / kKeyTile),
        1
    );
    v435_d128_gqa_e4m3_batch_head_fast_raster_production_bshd::main_kernel<<<
        grid,
        kThreads,
        0,
        stream
