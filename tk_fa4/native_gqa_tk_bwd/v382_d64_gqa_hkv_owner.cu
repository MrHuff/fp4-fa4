...
void check_common(
    ...
    TORCH_CHECK(
        ...
        "V382 D64 GQA Hkv-owner backward requires SM100"
    );
    TORCH_CHECK(
...
#pragma unroll
    ...
    if (lane == 0) {
        // BSHD row -> contiguous [B,H,1,S] statistics index.
...
std::vector<at::Tensor> backward_bf16_precomputed(
    ...
    const owner::hkv_globals globals{
    ...
    owner::launch_hkv_owner_bf16(globals, stream);
    owner::launch_hkv_finalize(
        dq_accum,
        dq,
        stream
    );
...
std::vector<at::Tensor> backward_bf16(
    ...
    // Validate every operand, statistics tensor, device, and scale before
    // the BSHD preprocess kernel dereferences out/dout.
    check_common(q, k, v, dout, lse_log2, delta, softmax_scale);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
    module.def(
        "backward_bf16",
        &backward_bf16,
        "V382 D64 GQA BF16 backward, Hkv-owned native BSHD"
    );
    module.def(
        "backward_bf16_precomputed",
        &backward_bf16_precomputed,
        "V382 D64 GQA BF16 backward, Hkv-owned BSHD with delta"
    );
}
