...
// Experimental B2/S4096-only producer-normalized common-row MXFP4-V fork.
// The v2 producer first computes one E8M0 anchor from the complete D128 BF16
// row amax, then makes a second BF16 pass to quantize the row directly to E2M1
// under that anchor.  There are no intermediate D32 codes and no requantization
// LUT.  All four physical scale slots repeat the common code.  The consumer
// directly restages packed bytes, reads slot zero for the row factor, and
// retains v490's unscaled mixed dP, dS factor application, overlap, and dV
// route.  This file is deliberately fail-closed to B2/S4096 and is not
// compatible with the existing per-D32 MX producer ABI.
namespace tkfa4::native_gqa_tk_bwd::v506_d128_gqa_mxfp4v_commonrow_e4m3do_b2_s4096_owner4_experimental_bshd {
...
static_assert(sizeof(row_scale_state) == 512);
...
// Common-row producer ABI:
//   payload [B,S,Hkv,D/2], with each D32 block occupying 16 packed bytes;
//   scales [B,S/128,Hkv,512], physical slot
//     (s&31)*16 + ((s>>5)&3)*4 + k_block;
//   one code is selected from the complete D128 BF16 row amax and repeated in
//     all four k_block slots;
//   payload is quantized directly from BF16 with E2M1 RNE under that code.
// Selecting the code before quantization requires the producer's second BF16
// row pass.  The consumer must not recompute the amax or requantize the bytes.

// Expand one already-normalized packed [S128,D/2] tile into the aligned E2M1
// byte container required by unscaled f8f6f4.  This is a direct one-time CTA
// restage: D128 amax selection and direct BF16-to-E2M1 quantization belong to
// the producer.
// The physical scale-page mapping remains the projection producer's exact
// [row_in_32, tile_in_block, k_block] mapping.
__device__ __forceinline__ void stage_persistent_commonrow_mxfp4_v(
    const globals &g,
    shared_storage &storage,
    ...
    // The producer repeats the common row code in all four D32 slots.  Read
    // slot zero only; the synthetic ABI gate authenticates repetition before
    // this experimental consumer is timed.
    if (threadIdx.x < kKeyTile) {
        ...
        const uint32_t common_code = scale_page[scale_base];
        const float power_of_two = common_code == 0
            ? 0.0f
            : __uint_as_float(common_code << 23);
...
void owner4_kernel(const __grid_constant__ globals g) {
    ...
    // Keep the v503 semaphore-initialization barrier as a compiler liveness
    // boundary.  Removing it raised the authenticated spill class from
    // 40/36 to 68/116 bytes even after consumer-side requantization was
    // deleted.
    __syncthreads();

    // Direct restage has no internal cross-warp dependency.  Its trailing CTA
    // barrier makes both packed V and row factors visible to all roles.
    stage_persistent_commonrow_mxfp4_v(
        g,
        storage,
        v_row_scale_shared,
        batch,
...
inline void launch(
    ...
    TORCH_CHECK(
        ...
        "v506 experimental MXFP4-V backward is fail-closed to B2/S4096"
    );
