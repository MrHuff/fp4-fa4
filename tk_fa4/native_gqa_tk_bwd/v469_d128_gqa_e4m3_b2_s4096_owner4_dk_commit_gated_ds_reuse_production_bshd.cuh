...
// B2/S4096 dominated-wait experiment derived only from frozen v468.
// Preserve its owner-4/direct-dK-dV topology, compact exact-rounded E4M3 P
// reuse, first-half quarter-dP pipeline, split-dV publication, score(next),
// dK-gated operand-stage reuse, and all numerical operations.  The same tensor
// lane issues dQ and its completion commit before dK and its completion commit.
// PTX defines tcgen05.commit as tracking all prior same-thread asynchronous
// TCGEN operations, so dK completion proves both consumers stopped reading old
// shared dS.  Remove only the compute warp's redundant old dq_ready wait before
// the retained old dk_ready wait; retain dq_ready declaration/init/commit and
// reducer waits, plus every dq_drained edge.  B1 falls back to v436; other B2
// sequence lengths fall back to v437.
namespace tkfa4::native_gqa_tk_bwd::v469_d128_gqa_e4m3_b2_s4096_owner4_dk_commit_gated_ds_reuse_production_bshd {
