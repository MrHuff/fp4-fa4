...
// B2/S4096 all-quarter-dP scheduling experiment derived only from frozen v460.
// Preserve its owner-4/direct-dK-dV topology, compact exact-rounded E4M3 P
// reuse, split-dV publication, score(next), first-half quarter pipeline, and
// all dQ/dK work.  Only second-half dP changes: replace its x32 load with the
// same owner-aligned x16 chunk-0/chunk-1 pipeline accepted in v460.  B1 falls
// back to v436; other B2 sequence lengths fall back to v437.
namespace tkfa4::native_gqa_tk_bwd::v464_d128_gqa_e4m3_b2_s4096_owner4_all_quarter_dp_split_dv_compact_p_reuse_production_bshd {
...
// offset ColumnHalf*64 + Chunk*16 from column zero, with each lane retaining
// eight float2 values.  Keep issue and wait separate so chunk-0 arithmetic can
// hide the second load without changing any numerical operation.
template <int ColumnHalf, int Chunk>
__device__ __forceinline__ void issue_dp_half_quarter(
    owner_aligned_fp32_quarter &destination,
    const attention_tmem_tile &source,
    int output_subtile
) {
    static_assert(ColumnHalf == 0 || ColumnHalf == 1);
    static_assert(Chunk == 0 || Chunk == 1);
    const uint32_t address =
        source.addr +
        (static_cast<uint32_t>(output_subtile * 16) << 16) +
        ColumnHalf * kColumnHalf +
        Chunk * (kColumnHalf / 4);
    asm volatile(
...
template <int ColumnHalf, int Chunk>
__device__ __forceinline__ void consume_ds_half_quarter(
    const owner_aligned_fp32_quarter &dp,
    const compact_probability_half &compact,
    shared_storage &storage,
    int output_subtile,
    ...
    const int query_column_base =
        ColumnHalf * kColumnHalf + lane_column_base +
        Chunk * kChunkColumns;
    const uint32_t shared_address = static_cast<uint32_t>(
        __cvta_generic_to_shared(&destination.data[0])
    );
...
#pragma unroll 1
        for (int local_head = 0; local_head < kHeadsPerOwner; ++local_head) {
            for (
                ...
                owner_aligned_fp32_quarter dp_third_quarter;
                issue_dp_half_quarter<1, 0>(
                    dp_third_quarter,
                    dp_tmem,
                    output_subtile
                );
                tensor_load_wait();
                owner_aligned_fp32_quarter dp_fourth_quarter;
                issue_dp_half_quarter<1, 1>(
                    dp_fourth_quarter,
                    dp_tmem,
                    output_subtile
                );
                // Match v460's accepted scheduling anchor: make the second
                // half's chunk-1 load visible before chunk-0 scalar work.
                // tcgen05.fence::before_thread_sync is sufficient; no warp
                // barrier participates in this register-only dependency.
                tensor_before_thread_sync();
                consume_ds_half_quarter<1, 0>(
                    dp_third_quarter,
                    probability_compact[1],
                    storage,
                    output_subtile,
