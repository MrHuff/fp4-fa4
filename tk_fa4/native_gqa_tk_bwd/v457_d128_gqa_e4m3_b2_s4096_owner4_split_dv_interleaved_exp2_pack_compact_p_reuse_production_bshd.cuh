...
// B2/S4096 composition derived byte-for-source from frozen v455.  Preserve
// its per-half P publication, split-dV handshake, early next-score issue,
// compact exact-rounded E4M3 P reuse, and half-0 dP preissue.  Import only
// v456's exact interleaved cadence: clamp/EX2 four FP32 pairs, immediately
// pack and publish the resulting two E4M3 words, then advance.  B1 falls back
// to v436; other B2 sequence lengths fall back to v437.
namespace tkfa4::native_gqa_tk_bwd::v457_d128_gqa_e4m3_b2_s4096_owner4_split_dv_interleaved_exp2_pack_compact_p_reuse_production_bshd {
...
__device__ __forceinline__ void make_probability_half_compact(
    ...
    // Preserve v455's element order and exact numerical operations while
    // importing v456's shorter four-pair EX2-to-publication live range.
#pragma unroll
