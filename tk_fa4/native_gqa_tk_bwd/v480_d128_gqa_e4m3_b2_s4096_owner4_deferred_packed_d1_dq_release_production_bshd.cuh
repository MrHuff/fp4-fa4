...
// B2/S4096 bounded dQ register-drain experiment derived only from v478.
// Preserve its owner-4/direct-dK-dV topology, exact compact-E4M3 arithmetic,
// tensor payload, head-boundary score overlap, and split TMEM/shared dQ
// handoff.  Keep D0 serial, retain exact-packed BF16 D1 in sixteen registers,
// then issue the paired D2/D3 x32 loads.  Release the aliased TMEM page before
// the deferred D1 and unchanged D2/D3 shared stores.  B1 falls back to v436;
// other B2 sequence lengths fall back to v437.
namespace tkfa4::native_gqa_tk_bwd::v480_d128_gqa_e4m3_b2_s4096_owner4_deferred_packed_d1_dq_release_production_bshd {
...
// D0 keeps v478's serial path.  D1 keeps the exact scale/round operation but
// retains its sixteen packed BF16 words in registers.  The paired D2/D3 loads
// then advertise complete TMEM capture before D1/D2/D3 shared stores execute.
...
#pragma unroll
        for (
            ...
            x32::store_bf16_x8(
                gradient_full_tile::idx(
                    ...
                    {
                        physical_row,
                        depth_chunk * prior::kDepthChunk + column,
                    }
                ),
                packed0,
                packed1,
    ...
    uint32_t deferred_packed_values[prior::kDepthChunk / 2];
    x32::load_tmem_owner_x32(
        values,
        source_row +
...
#pragma unroll
    for (
        ...
        deferred_packed_values[column / 2 + 0] =
            *reinterpret_cast<const uint32_t *>(&packed0);
        deferred_packed_values[column / 2 + 1] =
            *reinterpret_cast<const uint32_t *>(&packed1);
        deferred_packed_values[column / 2 + 2] =
            *reinterpret_cast<const uint32_t *>(&packed2);
        deferred_packed_values[column / 2 + 3] =
...
#pragma unroll
    for (
        ...
        asm volatile(
            ...
            :
            : "r"(deferred_packed_values[packed_column + 0]),
              "r"(deferred_packed_values[packed_column + 1]),
              "r"(deferred_packed_values[packed_column + 2]),
              "r"(deferred_packed_values[packed_column + 3]),
