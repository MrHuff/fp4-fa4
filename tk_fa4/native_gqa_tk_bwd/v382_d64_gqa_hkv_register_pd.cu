...
namespace owner =
    tkfa4::native_gqa_tk_bwd::v382_d64_hkv_register_pd;
namespace base_owner =
    tkfa4::native_gqa_tk_bwd::v382_d64_hkv_owner;
...
void check_operand_bshd(
    ...
    TORCH_CHECK(
        tensor.scalar_type() == dtype && tensor.dim() == 4 &&
            ...
            tensor.size(2) == heads &&
            tensor.size(3) == owner::control::kDepth,
        name,
...
void check_common(
    ...
    TORCH_CHECK(
        ...
        "V382 D64 GQA register-P/dP backward requires SM100"
    );
    TORCH_CHECK(
...
owner::hkv_globals make_globals(
    const at::Tensor &q,
    const at::Tensor &k,
    const at::Tensor &v,
    const at::Tensor &dout,
    ...
    return owner::hkv_globals{
        kittens::py::tensor_to_gl<owner::hkv_globals::q_gl>(q),
        kittens::py::tensor_to_gl<owner::hkv_globals::k_gl>(k),
        kittens::py::tensor_to_gl<owner::hkv_globals::v_gl>(v),
        kittens::py::tensor_to_gl<owner::hkv_globals::dout_gl>(dout),
        kittens::py::tensor_to_gl<owner::hkv_globals::hkv_out_gl>(dk),
        kittens::py::tensor_to_gl<owner::hkv_globals::hkv_out_gl>(dv),
        kittens::py::tensor_to_gl<owner::hkv_globals::dq_gl>(dq_accum),
...
void launch_precomputed_out_unchecked(
    ...
    const owner::hkv_globals globals = make_globals(
        q,
        k,
        v,
        dout,
    ...
    owner::launch_hkv_register_pd_bf16(globals, stream);
    base_owner::launch_hkv_finalize(dq_accum, dq, stream);
}
...
void preprocess_bshd_kernel(
    ...
    const int64_t input_base = row * owner::control::kDepth;
    float sum =
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
    module.def(
        "backward_bf16",
        &backward_bf16,
        "V382 D64 GQA BF16 backward with register-resident P/dP"
    );
    module.def(
        "backward_bf16_precomputed",
        &backward_bf16_precomputed,
        "Register-resident D64 GQA backward with precomputed statistics"
    );
    module.def(
        "backward_bf16_precomputed_out",
        &backward_bf16_precomputed_out,
        "Caller-owned register-resident D64 GQA backward"
    );
}
