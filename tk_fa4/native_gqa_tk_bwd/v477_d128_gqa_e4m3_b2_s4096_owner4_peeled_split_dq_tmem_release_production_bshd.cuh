...
// B2/S4096 dQ lifetime experiment derived only from v476.  Preserve its
// owner-4/direct-dK-dV topology, exact compact-E4M3 arithmetic, tensor payload,
// head-boundary score overlap, and split TMEM/shared dQ handoff.  Peel the
// final dQ depth chunk out of the drain loop so the early TMEM-release warp
// synchronization is straight-line code rather than an in-loop divergent
// branch.  B1 falls back to v436; other B2 sequence lengths fall back to v437.
namespace tkfa4::native_gqa_tk_bwd::v477_d128_gqa_e4m3_b2_s4096_owner4_peeled_split_dq_tmem_release_production_bshd {
...
// Only the final chunk advertises that every x32 source load has completed
// before its unchanged BF16 conversion and shared stores execute.
