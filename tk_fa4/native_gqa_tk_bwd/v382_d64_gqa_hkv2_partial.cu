...
namespace owner =
    tkfa4::native_gqa_tk_bwd::v382_d64_hkv2_partial;
...
void check_common(
    ...
    TORCH_CHECK(
        ...
        "V382 D64 GQA two-head partial backward requires SM100"
    );
    TORCH_CHECK(
...
std::vector<at::Tensor> backward_bf16_precomputed(
    ...
    owner::launch_hkv2_partial_bf16(globals, stream);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
    module.def(
        "backward_bf16",
        &backward_bf16,
        "V382 D64 GQA BF16 backward, two-head disjoint partial BSHD"
    );
    module.def(
        "backward_bf16_precomputed",
        &backward_bf16_precomputed,
        "V382 D64 GQA two-head partial BSHD backward with delta"
    );
}
