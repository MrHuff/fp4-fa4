...
// B2/S4096 dQ lifetime experiment derived only from v477.  Preserve its
// owner-4/direct-dK-dV topology, exact compact-E4M3 arithmetic, tensor payload,
// head-boundary score overlap, and split TMEM/shared dQ handoff.  Keep D0/D1
// load-wait-pack-store serial, then issue distinct D2/D3 x32 loads and retire
// both with one tensor-load wait.  Release the aliased TMEM page before the
// unchanged D2/D3 BF16 pack/shared stores.  B1 falls back to v436; other B2
// sequence lengths fall back to v437.
namespace tkfa4::native_gqa_tk_bwd::v478_d128_gqa_e4m3_b2_s4096_owner4_two_chunk_dq_tmem_release_production_bshd {
...
// The mathematical epilogue keeps v436's source-level operation sequence.
// The paired D2/D3 loads advertise that every x32 source load has completed
// before their unchanged BF16 conversions and shared stores execute.
