...
#include "v423_d128_gqa_e4m3_d2p2_async_production_bshd.cuh"
#include "../fp4_fa4_fwd/stage2_ex2_alu_helpers.cuh"

// D128 production backward preserving v422's double-buffered input and
// gradient-publication state machine.  Only probability replay changes: each
// owner word issues one native EX2 float2 first, computes the adjacent float2
// with the forward-derived packed degree-1 approximation while the SFU is in
// flight, then immediately converts and publishes the four E4M3 values.
namespace tkfa4::native_gqa_tk_bwd::v425_d128_gqa_e4m3_d1p2_async_production_bshd {

namespace predecessor =
    tkfa4::native_gqa_tk_bwd::v423_d128_gqa_e4m3_d2p2_async_production_bshd;
namespace core =
...
__device__ __forceinline__ float2 probability_log2_pair(
    ...
    // Production lstat already embeds +8, so clamp log2(256P) at +8.
    return d64::clamp_probability_log2(value);
}

__device__ __forceinline__ uint32_t probability_word_d1p2(
    const d64::owner_aligned_fp32_half &score,
    shared_storage &storage,
    int word,
    int key_row,
    ...
    const int degree1_pair = native_pair + 1;

    // Start the native pair before doing the independent degree-1 range
    // reduction/FMA chain.  Only these two float2 results remain live until
    // this four-value word is converted to E4M3.
    ...
    const float2 degree1_log2 = probability_log2_pair(
        score,
        storage,
        degree1_pair,
        key_row,
    ...
    const float2 degree1_probability =
        fp4pv_ex2_alu_emulation_degree1_f32x2(degree1_log2);
...
__device__ __forceinline__ void make_probability_half_d1p2(
...
void main_kernel(const __grid_constant__ globals g) {
    ...
    if (physical_warp == kLoaderWarp && lane == 0) {
        ...
        for (int iteration = 0; iteration < iteration_count; ++iteration) {
            ...
            // P publication has retired every score-TMEM reader.  Submit the
            // next score immediately from the alternate input stage, before
            // waiting for current dS/dQ/dK.  This is the main double-buffered
            // TMA/score overlap that the single-stage v421 cannot express.
            ...
            // All readers of this Q/dO input stage are now represented by
            // completion barriers.  Release the stage only after score, dK,
            // dP and dV have retired their shared-memory operands.
            wait(dk_ready, phase);
            wait(dp_ready, phase);
            wait(dv_ready, phase);
            tensor_after_thread_sync();
            ...
            // in TMEM [0,128).  Launch it at the exact reducer release point.
            // Retire dP(current) above before advancing the shared completion
            // semaphore to the next phase.
            if (has_next) {
...
inline void launch(
    ...
    v425_d128_gqa_e4m3_d1p2_async_production_bshd::main_kernel<<<
        grid,
        kThreads,
        0,
        stream
...
#pragma unroll
    for (int word_pair = 0; word_pair < kColumnHalf / 16; ++word_pair) {
        ...
        const uint32_t packed_first = probability_word_d1p2(
            score,
            storage,
            first_word,
            key_row,
        ...
        const uint32_t packed_second = probability_word_d1p2(
            score,
            storage,
            second_word,
            key_row,
