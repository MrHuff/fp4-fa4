...
V509_E5M2_DOUT_PUBLISHER_METADATA = {
    ...
    "dispatch": "B1_B2_B4_S4096_H32_D128_native_score_only",
...
def b300_lowp_bwd_extension_artifact_identity() -> dict[str, int | str]:
    """Return the stable identity of the loaded low-precision runtime image."""

    _ensure_lowp_bwd_extension()
    assert _C_b300_lowp_bwd is not None
    identity = getattr(
        _C_b300_lowp_bwd, "_tk_fa4_loaded_artifact_identity", None
    )
    if identity is None:
        loaded_path = getattr(_C_b300_lowp_bwd, "__file__", None)
        if loaded_path is None:
            raise RuntimeError(
                "loaded low-precision runtime does not expose an artifact path"
            )
        identity = _extension_file_identity(
            Path(loaded_path).resolve(strict=True)
        )
    return {**dict(identity), "module": LOWP_BWD_EXTENSION_MODULE}
...
def b300_project_gqa_d128_hierarchical_qkv_gradient_nvfp4(
    ...
    *,
    ...
    is authenticated for B=1, B=2, or B=4, or the private head-major
    ...
    if batch in (2, 4) and (sequence, q_heads, kv_heads) != (4096, 32, 8):
        raise ValueError(
...
def b300_require_v509_e5m2_dout_route(
    ...
    backward_batch = backward_metadata.get("batch")
    try:
        expected_backward = expected_extension_metadata(backward_batch)
    except (TypeError, ValueError) as error:
        raise RuntimeError(
            "v509 E5M2 dO route has an unsupported exact backward batch"
        ) from error
    backward_mismatches = {
        field: {
...
def b300_project_dout_unified_lowp_nvfp4_v509_e5m2(
    ...
    *,
    ...
    """Fuse exact B1/B2/B4 S4096/H32/D128 E5M2 dO for v509.
    ...
    batch = int(attention_output.shape[0])
    if batch not in (1, 2, 4):
        raise ValueError("v509 E5M2 dO publication requires batch 1, 2, or 4")
    expected_shape = (batch, 4096, 32, 128)
    if (
    ...
    expected_stats_bytes = 2 * batch * 32 * 4096 * torch.float32.itemsize
    if (
        stats_workspace.dtype != torch.uint8
        or not stats_workspace.is_cuda
        or not stats_workspace.is_contiguous()
    ...
    expected_stats_shape = (batch, 32, 1, 4096)
