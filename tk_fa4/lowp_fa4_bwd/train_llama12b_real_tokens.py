...
def _resolve_model_preset_options(
    ...
    d128_contract = (
        ...
        # D128 attention requires the numerically retained row-by-K16 Q/K
        # publication; fixed-head scales fail the full-depth training gate.
        ("mx_per_block_qk_scales", "--mx-per-block-qk-scales", True),
        ("fp8_per_block_qk_scales", "--fp8-per-block-qk-scales", True),
        (
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="training corpus (or set FA4_CORPUS_PATH)",
    )
    parser.add_argument(
        "--tokenizer",
        type=Path,
    ...
    for route in ("mx", "fp8"):
        ...
        parser.add_argument(
            ...
            help=(
                ...
                "logical row x K16; D128 NVFP4 keeps an independent "
                "accumulator-E4M3 backward, while D64 uses its E4M3 "
                "projection contract"
            ),
        )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "publisher; requires matched NVFP4 selected-route formats, true "
            "2D learned weight scaling, and accumulator-E4M3 backward "
            "publications"
        ),
    )
    ...
    for route in ("mx", "fp8"):
        ...
        if (
            ...
            and projection_format != "e4m3"
            and not (
                config.head_dim == 128 and projection_format == "nvfp4"
            )
        ):
            parser.error(
                f"--{route}-per-block-qk-scales requires the native "
                "D128 NVFP4 projection or the D64 E4M3 projection"
            )
    ...
    if config.head_dim == 64:
        ...
        for slot in selected_lowp_slots:
            if getattr(args, f"{slot}_qkv_projection_format") != "nvfp4":
                parser.error(
                    f"the production {slot.upper()} route requires "
                    f"--{slot}-qkv-projection-format=nvfp4 for D128"
                )
    ...
    shared_backward_runner_post_compile = None
    if mx_runtime is not None and fp8_runtime is not None:
        shared_backward_runner_post_compile = _share_matched_backward_runner(
            mx_runtime,
            fp8_runtime,
        )
    ...
    shared_backward_runner_post_training = None
    if mx_runtime is not None and fp8_runtime is not None:
        shared_backward_runner_post_training = _share_matched_backward_runner(
            mx_runtime,
            fp8_runtime,
        )
    ...
    result = {
        ...
        "configuration": {
            **config.__dict__,
            ...
            "shared_lowp_backward_runner_post_compile": (
                shared_backward_runner_post_compile
            ),
            "shared_lowp_backward_runner_post_training": (
                shared_backward_runner_post_training
            ),
            "timed_forward_dispatch_contracts": (
                timed_forward_dispatch_contracts
            ),
