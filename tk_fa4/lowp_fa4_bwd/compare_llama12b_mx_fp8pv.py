...
D64_NVFP4_PROJECTION_SYMBOL = (
    "project_qkv_gqa_d64_paired_unified_fp4_nvfp4_rope_packed"
)

D64_NVFP4_INTERLEAVED_PROJECTION_SYMBOL = (
    D64_NVFP4_PROJECTION_SYMBOL + "_interleaved_causal"
)

NVFP4_WEIGHT_QUANTIZATION_SYMBOL = "quantize_nvfp4_projection_weight"
...
def _require_d64_nvfp4_qkv_policy(
    args: argparse.Namespace,
    routes: tuple[str, ...] = ("mx", "fp8"),
) -> None:
    """Fail closed around the experimental native D64 NVFP4 publisher."""
    for route in routes:
    ...
    formats = {
        route: getattr(args, f"{route}_qkv_projection_format")
        for route in routes
    }
    if "nvfp4" not in formats.values():
        return
    if not args.experimental_d64_nvfp4_qkv:
        raise ValueError(
            "D64 NVFP4 QKV projection is experimental and requires "
            "--experimental-d64-nvfp4-qkv"
        )
    non_nvfp4 = [
        route for route, projection_format in formats.items()
        if projection_format != "nvfp4"
    ]
    if non_nvfp4:
        raise ValueError(
            "experimental D64 NVFP4 QKV projection requires every selected "
            f"low-precision route to use NVFP4; non-NVFP4 slots: {non_nvfp4}"
        )
    if args.projection_weight_scaling != "2d":
        raise ValueError(
            "experimental D64 NVFP4 QKV learned weights require true 16x16 "
            "scaling (--projection-weight-scaling=2d)"
        )
    represented_routes = [
        route for route in routes
        if getattr(args, f"{route}_backward_match_forward_operands")
    ]
    if represented_routes:
        raise ValueError(
            "experimental D64 NVFP4 QKV projection requires accumulator-E4M3 "
            "backward publications; disable backward-match-forward-operands "
            f"for slots {represented_routes}"
        )
    per_block_routes = [
        route for route in routes
    ...
    if per_block_routes:
        raise ValueError(
            "experimental D64 NVFP4 QKV projection does not publish D64 "
            "per-block Q/K scales; disable them for slots "
            f"{per_block_routes}"
        )
    if "mx" in routes and args.mx_experimental_split_v_backward:
        raise ValueError(
            "experimental D64 NVFP4 QKV projection requires "
...
def _required_projection_symbols(
    ...
    *,
    ...
    symbols: list[str] = []
    requires_nvfp4_weight_quantizer = False
    for route in routes:
        if route not in ("mx", "fp8"):
            raise ValueError(f"unsupported lowp route slot: {route!r}")
        projection_format = getattr(args, f"{route}_qkv_projection_format")
        if projection_format == "nvfp4":
            symbol = (
                D64_NVFP4_INTERLEAVED_PROJECTION_SYMBOL
                if route == "mx"
                else D64_NVFP4_PROJECTION_SYMBOL
            )
            if symbol not in symbols:
                symbols.append(symbol)
            requires_nvfp4_weight_quantizer = True
            continue
        if projection_format != "e4m3":
            raise ValueError(
                f"unsupported {route.upper()} QKV projection format: "
    ...
    if requires_nvfp4_weight_quantizer:
        symbols.append(NVFP4_WEIGHT_QUANTIZATION_SYMBOL)
...
def _timed_forward_dispatch_contracts(
    ...
    for route_name, runtime in runtimes.items():
        ...
        if (
            runtime.config.head_dim == 64
            and projection["format"] == "nvfp4"
        ):
            expected_attention = expected_attention_symbols[route_name]
            is_mx = route_name == "nvfp4_qk_mxfp4_pv"
            expected_launcher = (
                "_launch_forward_mx" if is_mx else "_launch_forward_fp8"
            )
            expected_projection_symbol = (
                D64_NVFP4_INTERLEAVED_PROJECTION_SYMBOL
                if is_mx
                else D64_NVFP4_PROJECTION_SYMBOL
            )
            expected_interleaved_kv = is_mx
            if not (
                runtime.config.head_dim == 64
                ...
                and runtime.projection_weight_scale_2d is True
                and runtime.projection_dgrad == "bf16"
                and runtime.backward_match_forward_operands is False
                and runtime.per_block_qk_scales is False
                and runtime.experimental_split_v_backward is False
                and runtime.causal_interleaved_kv
                is expected_interleaved_kv
                and callable(
                    getattr(
                        tk_interface,
                ...
                raise RuntimeError(
                    f"{route_name} did not validate the experimental native "
                    "D64 NVFP4 projection publication contract before timing"
                )
            if not (
                attention["dispatch"]
                ...
                raise RuntimeError(
                    f"{route_name} did not bind the required D64 "
            ...
            contracts[route_name] = {
                **contract,
                "d64_nvfp4_projection_publication": {
                    ...
                    "extension_entrypoint": expected_projection_symbol,
                    "projection_output_preallocated": False,
                    "timed_projection_output_allocations": True,
                    "allocations_shared_by_mx_and_fp8_routes": (
                        len(runtimes) == 2
                    ),
                    "native_causal_kv_layout": (
                        "interleaved" if is_mx else "ordinary"
                    ),
                    "paired_d64_head_publication": True,
                    "fixed_paired_head_qk_scales": True,
                    "per_block_qk_scales_available": False,
                    "projection_dgrad": "bf16",
                    "native_mxfp4_v_required": is_mx,
                    "native_feature_major_fp8_v_required": not is_mx,
                    "inactive_v_publication_omitted": is_mx,
        ...
        if not all(
            ...
            raise RuntimeError(
                f"{route_name} did not bind route-specific caller-owned "
                "projection symbols before timing"
            )
        route_suffix = (
            "_mx_forward_out"
        ...
        native_d128 = runtime.config.head_dim == 128
        expected_projection_format = "nvfp4" if native_d128 else "e4m3"
        if not (
            projection["format"] == expected_projection_format
            ...
            and runtime.qkv_projection is not None
            and (
                not native_d128
                or (
                    projection["experimental_native_nvfp4_caller_owned"]
                    is True
                    and projection["projection_forward_publication_path"]
                    == "caller_owned_route_selective_d128"
                    and projection["backward_publication_semantics"]
                    == "projection_accumulator_e4m3_qkv_shared_across_"
                    "pv_routes"
                    and getattr(runtime, "per_block_qk_scales", None) is True
                    and getattr(
            ...
            raise RuntimeError(
                f"{route_name} did not validate its construction-bound "
                f"{expected_projection_format.upper()} QKV projection "
                "before timing"
            )
        workspace_stream = (
            models[route_name].require_lowp_forward_workspace_stream()
        ...
        c = runtime.config
        qk_scale_head_multiplier = 2 if c.head_dim == 128 else 1
        expected_owners = {
            "q_payload": (
                4,
                [c.batch, c.q_heads, c.sequence, c.head_dim // 2],
                "torch.uint8",
            ),
            "k_payload": (
                6,
                [c.batch, c.kv_heads, c.sequence, c.head_dim // 2],
                "torch.uint8",
            ),
            "q_scale_pages": (
                8,
                [
                    c.batch,
                    c.sequence // 128,
                    c.q_heads * qk_scale_head_multiplier,
                    512,
                ],
                "torch.float8_e4m3fn",
            ),
            ...
            "k_scale_pages": (
                ...
                [
                    c.batch,
                    c.sequence // 64,
                    c.kv_heads * qk_scale_head_multiplier,
                    512,
                ],
                "torch.float8_e4m3fn",
            ),
            ...
            "v_mxfp4_payload": (
                12,
                [c.batch, c.kv_heads, c.head_dim, c.sequence // 2],
                "torch.float4_e2m1fn_x2",
            ),
            "v_mxfp4_scale_pages": (
                13,
                [c.batch, c.sequence // 128, c.kv_heads, 512],
                "torch.float8_e4m3fn",
            ),
            "v_backward_fp8": (
                20,
            ...
            "v_fp8_payload": (
                23,
                [c.batch, c.kv_heads, c.head_dim, c.sequence],
                "torch.float8_e4m3fn",
            ),
        }
        common_fields = {
        ...
        if native_d128:
            is_mx = route_name == "nvfp4_qk_mxfp4_pv"
            route_contract["d128_projection_publication"] = {
                "schema": "d128_nvfp4_bound_projection_preflight_v3",
                "python_entrypoint": (
                    "tk_fa4.interface."
                    "b300_bind_qkv_gqa_d128_unified_lowp_nvfp4_projection"
                ),
                "abi_validation_entrypoint": abi_validation_symbol,
                "checked_entrypoint": checked_symbol,
                "steady_state_entrypoint": unchecked_symbol,
                "projection_output_preallocated": True,
                "timed_projection_output_allocations": False,
                "route_neutral_workspace_schema_shared": len(runtimes) == 2,
                "native_mxfp4_v_required": is_mx,
                "native_feature_major_fp8_v_required": not is_mx,
                "inactive_v_publication_omitted": True,
                "shared_e4m3_backward_qkv": True,
        ...
        contracts[route_name] = route_contract
    return contracts
...
def _share_matched_backward_runner(
    ...
    """Verify construction retained one route-invariant backward."""
    _matched_backward_contracts(mx_runtime, fp8_runtime)
    return require_shared_backward_physical_identity(
        mx_runtime,
        fp8_runtime,
    )
    ...
    *,
    ...
    runtime = LowpAttentionRuntime(
        ...
        experimental_native_nvfp4_projection_out=(
            config.head_dim == 128 and qkv_projection_format == "nvfp4"
        ),
...
def _resolve_model_preset_options(
    ...
    d128_contract = (
        ...
        # Fixed-head D128 publication is fast in isolation but fails the
        # full-depth numerical gate. Keep row-by-K16 Q/K scaling symmetric
        # across MX and FP8 so their saved backward operands remain shared.
        (
            "mx_per_block_qk_scales",
            "--mx-per-block-qk-scales",
            True,
        ),
        (
            "fp8_per_block_qk_scales",
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "explicitly opt into the allocating native D64 NVFP4 QKV "
            "publisher; requires matched NVFP4 route formats, true 2D learned "
            "weight scaling, and accumulator-E4M3 backward publications"
        ),
    )
    parser.add_argument(
    ...
    if config.head_dim == 64:
        try:
            _require_d64_nvfp4_qkv_policy(args)
        except ValueError as error:
            parser.error(str(error))
        for route in ("mx", "fp8"):
            if (
    ...
    result = {
        "configuration": {
            ...
            **effective_attention_provenance,
            ...
            "experimental_d64_nvfp4_qkv": (
                args.experimental_d64_nvfp4_qkv
            ),
