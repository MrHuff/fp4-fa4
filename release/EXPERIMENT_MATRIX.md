...
| 8B matched distributed training | manifests rendered through `tools/render_fa4_training_config.py` and launched with `torchtitan.experiments.fa4.train` | snapshot exporter and `plot_causal_training.py` | Config rendering is runnable after data setup; a site rendezvous and clean distributed save/resume smoke are still required; committed historical curves are receipt-only |
| MXFP4-P/V divergence diagnostics | the explicit E4M3- or NVFP4-projection MXFP4 manifests | `plot_causal_training.py` | Config rendering is runnable; GPU/distributed validation remains pending and historical service histories are receipt-only |
