...
readonly RESULT_BASENAME="dolmino_dclm_a319_2k_d4q01_split_v_bwd_d1p2_clip1_seed20260818_20260820"
...
# Global-norm clipping follows the repository's convergence harness and is
# applied identically to BF16, MX, and FP8. The unclipped automatic d1/p2 and
# native d2/p0 attempts developed MX-only drift and became non-finite.
...
check_sha256 "69ae28f8ea3459b79a9e79d6aeb99270465ae2518f7d53dd417ac1377d942b28" \
...
if ! jq -e '
    ...
    and .configuration.gradient_clip_norm == 1.0
    and .configuration.gradient_clip_norm_type == 2.0
    and .configuration.gradient_clip_foreach == true
    and .configuration.gradient_error_if_nonfinite == true
    ...
    and (.records | all(.[]; all(.[]; .finite == true)))
    and (.records | all(.[]; all(.[];
        (.gradient_preclip_total_norm | isfinite)
        and (.gradient_was_clipped | type == "boolean")
