...
resume and complete the 100,000,595,968-token schedule.  The terminal update
saves the final checkpoint but is not a scheduled metric report.  We therefore
compare training at update 23,825 (99.93 billion tokens) and held-out
validation at update 23,840 (99.99 billion tokens), the last common scheduled
coordinates before the terminal update.
...
experiment over the completed 100-billion-token schedule.  The panel compares
the BF16 control with the retained FP8-P/V route; the lower panel magnifies
same-update validation from 60 billion tokens onward with an expanded vertical
axis.  Each curve is one trajectory, so the plot provides no uncertainty
estimate.}
\label{fig:llama8b-b4-matched-training}
\end{figure}
\FloatBarrier

At the last scheduled training report, BF16 and the FP8 route report losses of
2.3095 and 2.3613 and pre-clipping gradient norms of 0.0549 and 0.0532,
respectively.  At the final same-update held-out validation report, the losses
are 2.3048 and 2.3948, a gap of 0.0900.
...
gap to attention alone.  Across all 874 common logged coordinates from the end
of warmup through update 23,825, median throughput is 21,853
tokens/s/GPU for BF16 and 24,303 tokens/s/GPU for the FP8 route; the ratio
of medians is 1.112$\times$.  The paired throughput ratio has a 10th--90th
percentile range of 1.080--1.114$\times$.  This aggregation excludes no
...
all 874 common post-warmup observations.  Input stalls and checkpoint windows
...
over the completed 100-billion-token schedule, but not
statistical-equivalence claims or run-to-run uncertainty estimates.
