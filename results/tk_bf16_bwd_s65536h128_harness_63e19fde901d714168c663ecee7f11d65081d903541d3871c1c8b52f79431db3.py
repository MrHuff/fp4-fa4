...
def build_manifest(args: argparse.Namespace) -> dict:
    ...
    return {
        ...
        "git_head_tree": git_command("rev-parse", "HEAD^{tree}").strip(),
...
def runtime_module_provenance() -> dict:
    ...
    flash_attention_tree = subprocess.run(
        ("git", "-C", str(FLASH_ATTENTION), "rev-parse", "HEAD^{tree}"),
        capture_output=True,
        text=True,
    )
    flash_attention_status = subprocess.run(
        (
            "git",
            "-C",
            str(FLASH_ATTENTION),
    ...
    flash_interface_path = Path(flash_attn_cute_interface_module.__file__).resolve()
    cutlass_module_path = Path(cutlass_module.__file__).resolve()
    loaded_cutlass_version = getattr(cutlass_module, "__version__", None)
    environment_cutlass_version = distribution_version("nvidia-cutlass-dsl")
    environment_libs_base_version = distribution_version(
        "nvidia-cutlass-dsl-libs-base"
    )
    artifact_cutlass_version = local_artifact_distribution_version(
        "nvidia-cutlass-dsl"
    )
    artifact_libs_base_version = local_artifact_distribution_version(
        "nvidia-cutlass-dsl-libs-base"
    )
    return {
        "flash_attn_cute_interface": {
            "module_path": str(flash_interface_path),
            "module_sha256": sha256_file(flash_interface_path),
            "repository_head": (
            ...
            "repository_head_tree": (
                flash_attention_tree.stdout.strip()
                if flash_attention_tree.returncode == 0
                else None
            ),
            "tracked_worktree_dirty": (
                bool(flash_attention_status.stdout.strip())
                if flash_attention_status.returncode == 0
                else None
            ),
        },
        "cutlass_dsl": {
            "module_path": str(cutlass_module_path),
            "module_sha256": sha256_file(cutlass_module_path),
            "loaded_module_version": loaded_cutlass_version,
            "environment_distribution_version": environment_cutlass_version,
            "environment_libs_base_distribution_version": (
                environment_libs_base_version
            ),
            "vendored_artifact_distribution_version": artifact_cutlass_version,
            "vendored_artifact_libs_base_distribution_version": (
                artifact_libs_base_version
            ),
            "module_version": loaded_cutlass_version,
            "resolved_distribution_version": environment_cutlass_version,
            "resolved_libs_base_distribution_version": (
                environment_libs_base_version
            ),
            "local_artifact_distribution_version": artifact_cutlass_version,
...
def contract_fields(
    *,
    ...
    objective_evaluated: bool = False,
    ...
    return {
        ...
        "overall_contract_scope": overall_contract_scope,
...
def run_onecall(args: argparse.Namespace, extension) -> dict:
    ...
    return {
        ...
        **contract_fields(correctness_pass=child_error_pass),
...
def run_stress(args: argparse.Namespace, extension) -> dict:
    ...
    for call_idx in range(1, args.calls):
        ...
        for index, (name, value, baseline) in enumerate(
            ...
            if drift > drift_limit and first_drift[index] is None:
                ...
                first_drift[index] = {
                    ...
                    "changed_gt_drift_limit": int(
                        (difference > drift_limit).sum()
                    ),
                }
        torch.cuda.synchronize()
    ...
    return {
        ...
        **contract_fields(
            correctness_pass=(
                finite_pass and drift_pass and reference_error_pass
            )
        ),
        "gate_checks": {
...
def run_timing(args: argparse.Namespace, extension) -> dict:
    ...
    reference = call_cute(data)
    torch.cuda.synchronize()
    child_check = call_tk(child_fn, data, args.seqlen)
    torch.cuda.synchronize()
    child_rows = metrics(child_check, reference)
    correctness_pass = error_metrics_pass(child_rows)
    del child_check, reference
    ...
    gate_pass = correctness_pass and retention_pass
    return {
        **common_payload(args),
        "complete": True,
        "state": "finished",
        "gate_pass": gate_pass,
        "gate_scope": "correctness_and_retention",
        **contract_fields(
            correctness_pass=correctness_pass,
            retention_pass=retention_pass,
...
def run_compare(args: argparse.Namespace, extension) -> dict:
    ...
    return {
        **common_payload(args),
        ...
        "gate_scope": "correctness",
        **contract_fields(
            correctness_pass=child_error_pass,
            objective_evaluated=True,
            objective_beats_cute=objective_beats_cute,
        ),
        "gate_checks": {
...
def run_profile(args: argparse.Namespace, extension) -> dict:
    ...
    return {
        **common_payload(args),
        "complete": True,
        "state": "finished",
        "gate_pass": finite_pass,
        "gate_scope": "finite_only",
        "correctness_pass": None,
        "retention_evaluated": False,
        "retention_pass": None,
        "objective_evaluated": False,
