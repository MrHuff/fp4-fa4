...
// Optimized Fused RMSNorm + SiLU Activation + FP4 Quantization V2
// Key optimization: computes block absmaxes during first pass, avoiding redundant computation

// Full version with external scratch buffer
void launch_fused_rmsnorm_act_quant_v2(
    const nv_bfloat16* x,           // Input [rows, cols]
    const nv_bfloat16* w,           // RMSNorm gain [cols]
    float epsilon,
    int rows, int cols,
