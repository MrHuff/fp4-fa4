...
typename Gemm::Arguments args_from_options_nv(void* D_ptr,
                                           void* D_sf_ptr,
                                           void const* A_ptr,
                                           void const* B_ptr,
                                           ...
                                           int32_t M, int32_t N, int32_t K)
{
    ...
    if constexpr (IsBlockScaleSupported) {
        arguments.epilogue.thread.block_scale_factor_ptr = static_cast<cutlass::float_ue4m3_t*>(D_sf_ptr);
        arguments.epilogue.thread.norm_constant_ptr      = static_cast<ElementAccumulator const*>(global_scale_ptr);;
    }

    return arguments;
}

void runGemmNv(void* D_ptr,
             void* D_sf_ptr,
             void const* A_ptr,
             void const* B_ptr,
             void const* global_scale_ptr,
             ...
             cudaStream_t stream)
{
    ...
    auto arguments =
        args_from_options_nv(D_ptr, D_sf_ptr, A_ptr, B_ptr, global_scale_ptr, M, N, K);
...
void fusedQuantizeNvAbsMax_host_sm100(
                                      ...
                                      cudaStream_t stream)
{
#if defined(TARGET_CUDA_ARCH) && TARGET_CUDA_ARCH == 100
    int32_t M = A_numel / 128;
    int32_t N = B_size1;
    int32_t K = 128;

    runGemmNv(D_ptr, D_sf_ptr, A_ptr, B_ptr, global_scale_ptr, M, N, K, stream);
#else
    // Fallback to SM100 logic anyway for now
    int32_t M = A_numel / 128;
    int32_t N = B_size1;
    int32_t K = 128; // This seems fixed in the kernel?
    // Wait, K=128 for quantization tile?
