...
//
// Optimized Fused MXNorm (Block-Max RMS) + SiLU + FP4 Quantization Kernel
// "Fast Block" version: Hardcoded for Block-Max estimation.
...
// -------------------------------------------------------------------------
// MXNorm Fast Kernel (Block-Max Only)
// -------------------------------------------------------------------------

template<int BLOCK_SIZE = 256, bool USE_FOUR_SIX = true>
__global__ void fused_mxnorm_fast_block_kernel(
    ...
    // ===== PHASE 1: Statistics Collection (Block-Max Mode) =====
    // We compute block maxes for quantization ANYWAY.
    // In this mode, we use THESE block maxes to estimate the row RMS.
    ...
    // Reduce for inv_rms (Block-Max Estimate)
    // RMS of block_maxes
    ...
    // Done! We avoided the per-pixel sum_sq loop entirely.
...
void launch_fused_mxnorm_fast_block(
    ...
    if (use_four_six) {
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            &max_blocks_per_sm,
            fused_mxnorm_fast_block_kernel<BLOCK_SIZE, true>,
            BLOCK_SIZE, smem_size
        );
    } else {
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            &max_blocks_per_sm,
            fused_mxnorm_fast_block_kernel<BLOCK_SIZE, false>,
            BLOCK_SIZE, smem_size
        );
    }
    ...
    if (use_four_six) {
        cudaLaunchCooperativeKernel(
            (void*)fused_mxnorm_fast_block_kernel<BLOCK_SIZE, true>,
            dim3(rows), dim3(BLOCK_SIZE),
            args, smem_size, nullptr
        );
    } else {
        cudaLaunchCooperativeKernel(
            (void*)fused_mxnorm_fast_block_kernel<BLOCK_SIZE, false>,
            dim3(rows), dim3(BLOCK_SIZE),
            args, smem_size, nullptr
        );
    }
