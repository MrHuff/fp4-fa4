...
from tk_fa4.lowp_fa4_bwd.backward_contract import (
    ...
    shared_backward_physical_identity,
)
...
def _backward(offset: int) -> SimpleNamespace:
    return SimpleNamespace(
        workspace_torch=_Pointer(offset),
        dq=_Pointer(offset + 1),
        dk=_Pointer(offset + 2),
        dv=_Pointer(offset + 3),
        dk_partials=_Pointer(offset + 4),
        dv_partials=_Pointer(offset + 5),
        kernel=object(),
        compiled=object(),
    )
...
class _Runtime:
    def __init__(
        ...
        control: object,
        paired_rope: _Pointer,
        gradient_global_scale: _Pointer,
        contract: dict[str, object] | None = None,
    ) -> None:
        self.config = config
        self.rope = (_Pointer(202), _Pointer(203))
        self.backward = backward
        self.control = control
        self.paired_rope = paired_rope
        self.gradient_global_scale = gradient_global_scale
        self.contract = _contract() if contract is None else contract
        self.qkv_projection_format = "e4m3"
        self.q_quant_scale = 2.25
        self.k_quant_scale = 2.0
        self.per_block_qk_scales = True
        self.adaptive_qk_weight_scales = False

    def backward_contract(self) -> dict[str, object]:
        return self.contract


def _shared_runtimes(config: object) -> tuple[_Runtime, _Runtime]:
    backward = _backward(100)
    control = object()
    paired_rope = _Pointer(200)
    gradient_global_scale = _Pointer(201)
    reference = _Runtime(
        config,
        backward,
        control,
    ...
    candidate.rope = reference.rope
    reference.backward_execution_runtime = reference
    candidate.backward_execution_runtime = reference
    return reference, candidate


def _llama_bind_function(lowp_attention_type: type[object]):
    module = ast.parse(RUNTIME.read_text())
    llama = next(
        node
        for node in module.body
        if isinstance(node, ast.ClassDef) and node.name == "Llama12B"
    )
    bind = next(
        node
        for node in llama.body
        if isinstance(node, ast.FunctionDef)
        and node.name == "bind_lowp_attention_runtime"
    )
    ...
    namespace: dict[str, object] = {
        "LowpAttention": lowp_attention_type,
    ...
    exec(compile(executable, str(RUNTIME), "exec"), namespace)
    return namespace["bind_lowp_attention_runtime"]
...
def test_fused_rmsnorm_backward_policy_mismatch_fails_with_field() -> None:
    candidate = _contract()
    candidate["autograd"] = {
    ...
    with pytest.raises(RuntimeError) as error:
        require_matching_backward_contracts(
            {"reference": _contract(), "candidate": candidate}
        )
    assert (
        "autograd.experimental_fused_attention_rmsnorm_nvfp4"
        in str(error.value)
    )
...
def test_shared_backward_identity_covers_retained_outputs_and_compilation() -> None:
    reference, candidate = _shared_runtimes(object())
    checks = shared_backward_physical_identity(reference, candidate)
    assert all(checks.values())
    assert {
        "shared_backward_execution_runtime_object",
        "shared_runner_object",
        "shared_control_module_object",
        "shared_rope_tuple_object",
        "shared_rope_cos_data_ptr",
        "shared_rope_sin_data_ptr",
        "shared_workspace_object",
        "shared_workspace_data_ptr",
        "shared_dq_object",
        "shared_dq_data_ptr",
        "shared_dk_partials_object",
        "shared_dk_partials_data_ptr",
        "shared_kernel_object",
        "shared_compiled_callable_object",
    } <= checks.keys()


def test_shared_backward_identity_fails_closed_for_distinct_runner() -> None:
    config = object()
    reference, candidate = _shared_runtimes(config)
    candidate.backward = _backward(300)
    with pytest.raises(RuntimeError) as error:
        require_shared_backward_physical_identity(reference, candidate)
    message = str(error.value)
    assert "shared_runner_object" in message
    assert "shared_workspace_data_ptr" in message
    assert "shared_compiled_callable_object" in message


def test_shared_backward_identity_fails_closed_for_distinct_runtime_owner() -> None:
    reference, candidate = _shared_runtimes(object())
    candidate.backward_execution_runtime = candidate
    with pytest.raises(RuntimeError) as error:
        require_shared_backward_physical_identity(reference, candidate)
    assert "shared_backward_execution_runtime_object" in str(error.value)


def test_shared_backward_identity_fails_closed_for_distinct_unpacked_rope() -> None:
    reference, candidate = _shared_runtimes(object())
    candidate.rope = (_Pointer(202), _Pointer(203))
    with pytest.raises(RuntimeError) as error:
        require_shared_backward_physical_identity(reference, candidate)
    message = str(error.value)
    assert "shared_rope_tuple_object" in message
    assert "shared_rope_cos_object" in message
    assert "shared_rope_sin_object" in message


def test_trainer_rechecks_physical_identity_after_compile_and_training() -> None:
    source = TRAINER.read_text()

    assert source.count("_share_matched_backward_runner(") >= 3
    assert '"shared_lowp_backward_runner_post_compile"' in source
    assert '"shared_lowp_backward_runner_post_training"' in source


def test_only_lowp_runtime_borrows_shared_backward_rope() -> None:
    source = RUNTIME.read_text()
    bf16_attention = source.split("class BF16Attention", 1)[1].split(
        "class LowpAttentionRuntime", 1
    )[0]
    lowp_runtime = source.split("class LowpAttentionRuntime", 1)[1].split(
        "class _LowpAttentionFunction", 1
    )[0]

    assert "self.rope = rope" in bf16_attention
    assert "shared_backward_runtime" not in bf16_attention
    assert "else shared_backward_runtime.rope" in lowp_runtime
    assert "else shared_backward_runtime.backward_execution_runtime" in (
        lowp_runtime
    )
    assert "ctx.runtime = runtime.backward_execution_runtime" in source
...
def test_shared_runtime_validates_complete_logical_and_physical_backward() -> None:
    source = RUNTIME.read_text()
    shared_branch = source.split(
        "if shared_backward_runtime is not None:", 2
    )[-1].split("return", 1)[0]
    assert "shared.backward_contract()" in shared_branch
    assert "self.backward_contract()" in shared_branch
    assert "require_matching_backward_contracts(" in shared_branch
    assert "require_shared_backward_physical_identity(shared, self)" in (
        shared_branch
    )
...
def test_layer_forward_workspace_is_route_neutral_nonpersistent_scratch() -> None:
    ...
    assert "_require_forward_workspace_same_stream(" in lowp_forward
    assert "workspace.publication_state.begin_forward(" in lowp_forward
...
def test_runtime_crossover_accepts_one_logical_and_physical_backward() -> None:
    class Attention:
    ...
    class Layer:
        def __init__(self, runtime: _Runtime) -> None:
            self.attention = Attention(runtime)

    config = SimpleNamespace(layers=2)
    reference, candidate = _shared_runtimes(config)
    model = SimpleNamespace(
        config=config,
        layers=[Layer(reference), Layer(reference)],
        lowp_attention_runtime=reference,
    )
    bind = _llama_bind_function(Attention)
    assert bind(model, candidate) == 2
    assert model.lowp_attention_runtime is candidate
    assert all(
        layer.attention.runtime is candidate for layer in model.layers
    )


def test_runtime_crossover_contract_mismatch_is_atomic() -> None:
    class Attention:
        def __init__(self, runtime: _Runtime) -> None:
            self.runtime = runtime
    ...
    config = SimpleNamespace(layers=2)
    reference, candidate = _shared_runtimes(config)
    projection = dict(candidate.contract["projection"])
    projection["v_backward_source"] = "represented_mxfp4_codes"
    candidate.contract["projection"] = projection
    model = SimpleNamespace(
        config=config,
        layers=[Layer(reference), Layer(reference)],
        lowp_attention_runtime=reference,
    ...
    assert "projection.v_backward_source" in str(error.value)
    assert model.lowp_attention_runtime is reference
    assert all(
        layer.attention.runtime is reference for layer in model.layers
    )


def test_runtime_crossover_physical_mismatch_is_atomic() -> None:
    class Attention:
        def __init__(self, runtime: _Runtime) -> None:
            self.runtime = runtime
    ...
    with pytest.raises(ValueError) as error:
        bind(model, candidate)
    assert "shared_runner_object" in str(error.value)
    assert model.lowp_attention_runtime is reference
    assert all(
        layer.attention.runtime is reference for layer in model.layers
    )
