...
D64_NVFP4 = "project_qkv_gqa_d64_paired_unified_fp4_nvfp4_rope_packed"
D64_NVFP4_INTERLEAVED = D64_NVFP4 + "_interleaved_causal"
NVFP4_WEIGHT_QUANTIZER = "quantize_nvfp4_projection_weight"
...
def test_production_matched_cli_defaults_are_explicit() -> None:
    ...
    expected = {
        ...
        "--experimental-d64-nvfp4-qkv": False,
...
def test_d64_nvfp4_policy_is_explicit_and_fail_closed() -> None:
    namespace = _execute_functions(
        ("_require_d64_nvfp4_qkv_policy",),
        {"argparse": argparse},
    )
    require_policy = namespace["_require_d64_nvfp4_qkv_policy"]
    args = SimpleNamespace(
        experimental_d64_nvfp4_qkv=False,
        mx_qkv_projection_format="e4m3",
        fp8_qkv_projection_format="e4m3",
    ...
    args.mx_qkv_projection_format = "nvfp4"
    args.fp8_qkv_projection_format = "nvfp4"
    with pytest.raises(ValueError, match="--experimental-d64-nvfp4-qkv"):
        require_policy(args)

    args.experimental_d64_nvfp4_qkv = True
    args.fp8_qkv_projection_format = "e4m3"
    with pytest.raises(ValueError, match="every selected"):
        require_policy(args)

    args.fp8_qkv_projection_format = "nvfp4"
    args.projection_weight_scaling = "1d"
    with pytest.raises(ValueError, match="true 16x16"):
        require_policy(args)

    args.projection_weight_scaling = "2d"
    with pytest.raises(ValueError, match="backward-match-forward-operands"):
        require_policy(args)

    args.mx_backward_match_forward_operands = False
    args.fp8_backward_match_forward_operands = False
    with pytest.raises(ValueError, match="per-block Q/K"):
        require_policy(args)
    ...
    with pytest.raises(
        ValueError,
        match="--no-mx-experimental-split-v-backward",
    ):
        require_policy(args)

    args.mx_experimental_split_v_backward = False
    require_policy(args)


def test_real_token_trainer_uses_the_same_d64_nvfp4_opt_in_policy() -> None:
    source = TRAINER.read_text()
    assert "_require_d64_nvfp4_qkv_policy," in source
    assert '"--experimental-d64-nvfp4-qkv"' in source
    assert (
        "_require_d64_nvfp4_qkv_policy(args, selected_lowp_slots)"
        in source
    )
    assert '"experimental_d64_nvfp4_qkv": (' in source


def test_d64_nvfp4_symbols_authenticate_both_native_layouts_and_weight_pack() -> None:
    namespace = _execute_functions(
        ("_required_projection_symbols",),
        {
            "argparse": argparse,
            "E4M3_PAIRED_PROJECTION_SYMBOL": E4M3_BASE,
            "D64_NVFP4_PROJECTION_SYMBOL": D64_NVFP4,
            "D64_NVFP4_INTERLEAVED_PROJECTION_SYMBOL": (
                D64_NVFP4_INTERLEAVED
            ),
            "NVFP4_WEIGHT_QUANTIZATION_SYMBOL": NVFP4_WEIGHT_QUANTIZER,
        },
    )
    args = SimpleNamespace(
...
def test_d128_preset_resolves_without_changing_d64_defaults() -> None:
    ...
    assert d64.projection_weight_scaling == "1d"

    d128 = route_args()
    d128.mx_per_block_qk_scales = False
    d128.fp8_per_block_qk_scales = False
    namespace["_resolve_model_preset_options"](
        d128,
    ...
    assert d128.projection_weight_scaling == "2d"
    assert d128.mx_per_block_qk_scales is True
    assert d128.fp8_per_block_qk_scales is True
    ...
    for route in ("mx", "fp8"):
        disabled_perblock = route_args()
        setattr(disabled_perblock, f"{route}_per_block_qk_scales", False)
...
def test_timed_forward_contracts_require_exact_bound_route_symbols() -> None:
    ...
    with pytest.raises(RuntimeError, match="construction-bound E4M3 QKV"):
        invalid_mx = Runtime(
...
def test_timed_d128_contract_authenticates_bound_route_selective_workspace() -> None:
    namespace = _execute_functions(
        ("_timed_forward_dispatch_contracts",),
        {
            "Any": Any,
    ...
    class Runtime:
        def __init__(self, pv_format: str) -> None:
            ...
            self.qkv_projection = SimpleNamespace(per_block_qk_scales=True)
            self.qk_scales = SimpleNamespace(device="cuda:0")

        def forward_dispatch_contract(self) -> dict[str, object]:
            is_mx = self.pv_format == "mxfp4_e8m0_block32"
            abi_symbol = D128_NVFP4
            checked_symbol = abi_symbol + (
                "_mx_forward_out" if is_mx else "_fp8_forward_out"
            )
            unchecked_symbol = checked_symbol + "_unchecked"
            return {
                ...
                "qkv_projection": {
                    ...
                    "unchecked_symbol": unchecked_symbol,
                    "shape_bound_at_construction": True,
                    "first_call_full_abi_validation_complete": True,
                    "subsequent_call_path": (
                        "bound_exact_pybind_symbol_with_preallocated_"
                        "forward_workspace"
                    ),
                    ...
                    "preallocated_forward_workspace_abi_validated": True,
                    "validated_forward_workspace_count": self.config.layers,
                    "timed_forward_publication_allocation_fallback": False,
                    "preallocated_forward_workspace_ownership": (
                        "private_nonpersistent_layer_route_neutral_superset"
                    ),
    ...
    class Model:
        ...
        def lowp_forward_workspace_contract(self) -> dict[str, object]:
            ...
            owner_specs = {
                "q_payload": (
                    4,
                    [1, c.q_heads, c.sequence, c.head_dim // 2],
                    "torch.uint8",
                ),
                "k_payload": (
                    6,
                    [1, c.kv_heads, c.sequence, c.head_dim // 2],
                    "torch.uint8",
                ),
                "q_scale_pages": (
                    8,
                    [1, c.sequence // 128, c.q_heads * 2, 512],
                    "torch.float8_e4m3fn",
                ),
                "q_global_scale": (9, [1, c.q_heads], "torch.float32"),
                "k_scale_pages": (
                    10,
                    [1, c.sequence // 64, c.kv_heads * 2, 512],
                    "torch.float8_e4m3fn",
                ),
                "k_global_scale": (11, [1, c.kv_heads], "torch.float32"),
                "v_mxfp4_payload": (
                    12,
                    [1, c.kv_heads, c.head_dim, c.sequence // 2],
                    "torch.float4_e2m1fn_x2",
                ),
                "v_mxfp4_scale_pages": (
                    13,
                    [1, c.sequence // 128, c.kv_heads, 512],
                    "torch.float8_e4m3fn",
                ),
                "v_backward_fp8": (
                    20,
                    [1, c.sequence, c.kv_heads, c.head_dim],
                    "torch.float8_e4m3fn",
                ),
                "q_backward_fp8": (
                    21,
                    [1, c.sequence, c.q_heads, c.head_dim],
                    "torch.float8_e4m3fn",
                ),
                "k_backward_fp8": (
                    22,
                ...
                "v_fp8_payload": (
                    23,
                    [1, c.kv_heads, c.head_dim, c.sequence],
                    "torch.float8_e4m3fn",
                ),
            }
            common_fields = {
            ...
            layers = []
            for layer_index in range(c.layers):
                owners = {
                    name: {
                        "slot": slot,
                        "data_ptr": 10000 + layer_index * 100 + slot,
                        "allocation_data_ptr": (
                            10000 + layer_index * 100 + slot
                        ),
                        "pointer_stable_since_allocation": True,
                        "shape": shape,
                        "dtype": dtype,
                ...
                layers.append(
                    {
                        ...
                        "requires_forward_workspace": True,
                        "forward_workspace_abi_validated": True,
                        "validated_forward_workspace_count": c.layers,
                    }
                )
            return {
                "schema": "lowp_model_forward_workspaces_v2",
                "layer_count": c.layers,
                "owner_count": len(owner_specs) * c.layers,
    ...
    contracts = helper(mx, fp8, Model(mx), Model(fp8))
    for contract in contracts.values():
        publication = contract["d128_projection_publication"]
        assert publication["projection_output_preallocated"] is True
        assert publication["timed_projection_output_allocations"] is False
        assert publication["route_neutral_workspace_schema_shared"] is True
        assert publication["shared_e4m3_backward_qkv"] is True
        assert publication["per_block_qk_scales"] is True
        assert publication["qk_scale_geometry"] == "row_by_k16"
        assert publication["inactive_v_publication_omitted"] is True
        assert (
    ...
    assert (
        contracts["nvfp4_qk_mxfp4_pv"]["d128_projection_publication"]
        ["native_mxfp4_v_required"]
        is True
    )
    assert (
        contracts["nvfp4_qk_fp8_pv_exact"]["d128_projection_publication"]
        ["native_feature_major_fp8_v_required"]
        is True
    )

    mx_only = helper(mx, None, Model(mx), None)
    assert set(mx_only) == {"nvfp4_qk_mxfp4_pv"}
    assert (
        mx_only["nvfp4_qk_mxfp4_pv"]["d128_projection_publication"]
        ["route_neutral_workspace_schema_shared"]
        is False
    )
    fp8_only = helper(None, fp8, None, Model(fp8))
    ...
    mx.per_block_qk_scales = False
    with pytest.raises(RuntimeError, match="construction-bound NVFP4 QKV"):
        helper(mx, fp8, Model(mx), Model(fp8))
    mx.per_block_qk_scales = True
    mx.qkv_projection.per_block_qk_scales = False
    with pytest.raises(RuntimeError, match="construction-bound NVFP4 QKV"):
        helper(mx, fp8, Model(mx), Model(fp8))
    mx.qkv_projection.per_block_qk_scales = True
...
def test_timed_d64_nvfp4_contract_reports_allocating_native_publication() -> None:
    namespace = _execute_functions(
        ("_timed_forward_dispatch_contracts",),
        {
            "Any": Any,
    ...
    class Runtime:
        def __init__(self, pv_format: str) -> None:
            is_mx = pv_format == "mxfp4_e8m0_block32"
            self.pv_format = pv_format
            self.config = SimpleNamespace(head_dim=64)
            self.backward_forward_mx_probability_scale_handoff = False
            self.qkv_projection = None
            self.projection_weight_scale_2d = True
            self.projection_dgrad = "bf16"
            self.backward_match_forward_operands = False
            self.per_block_qk_scales = False
            self.experimental_split_v_backward = False
            self.causal_interleaved_kv = is_mx
    ...
    mx_publication = contracts["nvfp4_qk_mxfp4_pv"][
        "d64_nvfp4_projection_publication"
    ]
    fp8_publication = contracts["nvfp4_qk_fp8_pv_exact"][
        "d64_nvfp4_projection_publication"
    ]
    assert mx_publication["extension_entrypoint"] == D64_NVFP4_INTERLEAVED
    assert mx_publication["native_causal_kv_layout"] == "interleaved"
    assert fp8_publication["extension_entrypoint"] == D64_NVFP4
    assert fp8_publication["native_causal_kv_layout"] == "ordinary"
    assert mx_publication["native_mxfp4_v_required"] is True
    assert mx_publication["native_feature_major_fp8_v_required"] is False
    assert mx_publication["inactive_v_publication_omitted"] is True
    assert fp8_publication["native_mxfp4_v_required"] is False
    assert fp8_publication["native_feature_major_fp8_v_required"] is True
    assert fp8_publication["inactive_v_publication_omitted"] is False
    assert mx_publication["projection_output_preallocated"] is False
    assert mx_publication["timed_projection_output_allocations"] is True
    assert mx_publication["projection_dgrad"] == "bf16"

    fp8.projection_dgrad = "nvfp4"
    with pytest.raises(RuntimeError, match="native D64 NVFP4"):
...
def test_matched_routes_share_physical_backward_and_common_operands() -> None:
    ...
    backward = SimpleNamespace(
        workspace_torch=Pointer(10),
        dq=Pointer(11),
        dk=Pointer(12),
        dv=Pointer(13),
        dk_partials=Pointer(14),
        dv_partials=Pointer(15),
        kernel=object(),
        compiled=object(),
    )
    control = object()
    rope = (Pointer(18), Pointer(19))
    paired_rope = Pointer(16)
    gradient_global_scale = Pointer(17)
    ...
    class Runtime:
        def __init__(self) -> None:
            ...
            self.rope = rope
            self.paired_rope = paired_rope
            self.gradient_global_scale = gradient_global_scale
    ...
    mx = Runtime()
    fp8 = Runtime()
    mx.backward_execution_runtime = mx
    fp8.backward_execution_runtime = mx
    before = (
        id(fp8.backward),
        id(fp8.control),
        id(fp8.rope),
        id(fp8.paired_rope),
        id(fp8.gradient_global_scale),
    )
    ...
    assert "shared_compiled_callable_object" in provenance
    assert "shared_dq_data_ptr" in provenance
    assert "shared_dk_partials_data_ptr" in provenance


def test_matched_backward_verifier_does_not_repair_distinct_runners() -> None:
    namespace = _execute_functions(
        ("_matched_backward_contracts", "_share_matched_backward_runner"),
        {
            "Any": Any,
    ...
    class Runtime:
        def __init__(self, offset: int) -> None:
            self.backward = SimpleNamespace(
                workspace_torch=Pointer(offset),
                dq=Pointer(offset + 1),
                dk=Pointer(offset + 2),
                dv=Pointer(offset + 3),
                dk_partials=Pointer(offset + 4),
                dv_partials=Pointer(offset + 5),
                kernel=object(),
                compiled=object(),
            )
            self.control = object()
            self.rope = (Pointer(offset + 8), Pointer(offset + 9))
            self.paired_rope = Pointer(offset + 6)
            self.gradient_global_scale = Pointer(offset + 7)
    ...
    fp8_objects = (
        ...
        fp8.rope,
        fp8.paired_rope,
        fp8.gradient_global_scale,
    )
    with pytest.raises(RuntimeError, match="physical identity mismatch"):
        namespace["_share_matched_backward_runner"](mx, fp8)
    assert fp8_objects == (
        fp8.backward,
        fp8.control,
        fp8.rope,
