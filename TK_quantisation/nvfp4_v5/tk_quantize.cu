...
tk_fused_norm_quantize(
    ...
    static int s_norm_quant_num_persistent = 0;
    int dshmem = tk_v5::persistent_norm_quant_smem_size(return_transpose);
    if (s_norm_quant_num_persistent == 0) {
        ...
        s_norm_quant_num_persistent = max_blocks * num_sms;
    }
    int num_persistent = std::min(s_norm_quant_num_persistent, total_tiles);
