...
static CachedInfo& get_cached() {
    ...
    if (!ci.initialized) {
        ...
        cudaFuncSetAttribute(mxfp4_v3_fused_kernel<QuantMode::DECODE>,
            cudaFuncAttributeMaxDynamicSharedMemorySize, ci.dshmem);
        cudaFuncSetAttribute(mxfp4_v3_fused_kernel<QuantMode::UP_1_8>,
            cudaFuncAttributeMaxDynamicSharedMemorySize, ci.dshmem);
        cudaFuncSetAttribute(mxfp4_v3_fused_kernel<QuantMode::UP_1_4>,
            cudaFuncAttributeMaxDynamicSharedMemorySize, ci.dshmem);
        cudaFuncSetAttribute(mxfp4_v3_fused_kernel<QuantMode::UP_3_8>,
            cudaFuncAttributeMaxDynamicSharedMemorySize, ci.dshmem);
...
mxfp4_quantize_for_gemm(torch::Tensor input, int mode) {
    switch (mode) {
        ...
        case 3: return mxfp4_quantize_for_gemm_impl<QuantMode::UP_1_8>(input);
        case 4: return mxfp4_quantize_for_gemm_impl<QuantMode::UP_1_4>(input);
        case 5: return mxfp4_quantize_for_gemm_impl<QuantMode::UP_3_8>(input);
...
template<QuantMode MODE>
std::tuple<torch::Tensor, torch::Tensor>
mxfp4_quantize_bshd128_row_out_impl(
    torch::Tensor input,
    torch::Tensor row_fp4,
    torch::Tensor row_sc
) {
    ...
    const int64_t H = input.size(2);
    const int64_t D = input.size(3);
    TORCH_CHECK(D == 128, "mxfp4_quantize_bshd128_row_out requires D=128");
    TORCH_CHECK(S % 128 == 0, "sequence length must be divisible by 128");
    TORCH_CHECK(B > 0 && H > 0 && S > 0);
    TORCH_CHECK(row_fp4.scalar_type() == torch::kFloat4_e2m1fn_x2);
    TORCH_CHECK(row_sc.scalar_type() == torch::kUInt8);
    ...
    auto stream = at::cuda::getCurrentCUDAStream();
    dim3 grid(static_cast<unsigned int>(S / 128), static_cast<unsigned int>(B * H));
    mxfp4_bshd128_row_delta_kernel<MODE, false><<<grid, THREADS, 0, stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(input.data_ptr()),
        nullptr,
        reinterpret_cast<fp4e2m1x2*>(row_fp4.data_ptr()),
        reinterpret_cast<uint8_t*>(row_sc.data_ptr()),
    ...
    auto err = cudaGetLastError();
    TORCH_CHECK(err == cudaSuccess, "mxfp4 bshd128 row quantize out: ", cudaGetErrorString(err));
    return std::make_tuple(row_fp4, row_sc);
}

std::tuple<torch::Tensor, torch::Tensor>
mxfp4_quantize_bshd128_row_out(
    torch::Tensor input,
    torch::Tensor row_fp4,
    torch::Tensor row_sc,
    int mode
    ...
    switch (mode) {
        case 1:
            return mxfp4_quantize_bshd128_row_out_impl<QuantMode::ENCODE>(
                input, row_fp4, row_sc);
        case 2:
            return mxfp4_quantize_bshd128_row_out_impl<QuantMode::DECODE>(
                input, row_fp4, row_sc);
        default:
            return mxfp4_quantize_bshd128_row_out_impl<QuantMode::RTE>(
                input, row_fp4, row_sc);
    }
}
...
mxfp4_quantize_bshd128_row_with_delta_out_impl(
    ...
    auto stream = at::cuda::getCurrentCUDAStream();
    dim3 grid(static_cast<unsigned int>(S / 128), static_cast<unsigned int>(B * H));
    mxfp4_bshd128_row_delta_kernel<MODE, true><<<grid, THREADS, 0, stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(input.data_ptr()),
        reinterpret_cast<const __nv_bfloat16*>(out.data_ptr()),
        reinterpret_cast<fp4e2m1x2*>(row_fp4.data_ptr()),
        reinterpret_cast<uint8_t*>(row_sc.data_ptr()),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("mxfp4_quantize_bshd128_row_out", &mxfp4_quantize_bshd128_row_out,
          "MXFP4 v3 direct quantize BF16 [B,S,H,128] rows into preallocated outputs",
          py::arg("input"), py::arg("row_fp4"), py::arg("row_sc"), py::arg("mode") = 0);
